...
int spectator_flag =0; //hle;
int g_TeamplayClient = 0; //hle;
float testDuck = 0; //used to disable duck script
//float g_DecentHack=0;


#include "cvardef.h"
/*
#ifdef CLIENT_DLL
extern float pm_time;
float pm_starttime;
//cvar_t * p_bunnyhop = NULL; 
//cvar_t * p_autojump = NULL; 
#else
float pm_time=0;

extern cvar_t bunnyhop;
extern cvar_t autojump;
//cvar_t * p_bunnyhop = &bunnyhop;
//cvar_t * p_autojump = &autojump;
#endif*/

#ifdef CLIENT_DLL 
extern float pm_time; 
extern float startTime;
extern float lastUnDuck;
#endif
...
{
//	qboolean this_fucking_shit_doest_work;
	int	fWalking;
	float fvol;
	vec3_t knee;
	vec3_t feet;
		...
		( speed >= velwalk || !pmove->flTimeStepSound ) )
	{
		...
		// THIS IS WHERE WE TURN OFF FOOT STEPS FOR OBSERVER MODE
		

	//	this_fucking_shit_doest_work = atoi( pmove->PM_Info_ValueForKey( pmove->physinfo, "if_fucking_spawned" ) ) == 1 ? true : false;
		
	//	if(this_fucking_shit_doest_work == false)	
			
		PM_PlayStepSound( step, fvol ); // FUCKING HACK FOR THIS GAY OBSERVER MODE FOOTSTEP SHIT!
...
usedown:
	...
	//pmove->Con_DPrintf("Moving: %f", Length( pmove->velocity));
...
void PM_AirAccelerate (vec3_t wishdir, float wishspeed, float accel)
{
	...
	//chose to reset timer in air since it doesnt matter once the player is on the ground
	#ifdef CLIENT_DLL
	//do a check to see if holding jump
	if((pmove->cmd.buttons & IN_JUMP) == 0) //if not pushing jump
	{
		startTime = pm_time; //set startTime to current clients time
	}
	#else
	//chose to use Sys_FloatTime() so it would match with the clients pm_time
	if((pmove->cmd.buttons & IN_JUMP) == 0) //if not pushing jump
	{
		pmove->fuser4 = pmove->Sys_FloatTime(); //set fuser4 to system time in seconds
	}
	#endif
...
void PM_Duck( void )
{
	...
	//dont allow script to duck multiple times really fast
	/*if(pmove->oldbuttons & IN_DUCK)// if not the first time pushing duck
	{
		#ifdef CLIENT_DLL
		if((pm_time - lastUnDuck) < 0.05)
			return;
		#else
		if((pmove->Sys_FloatTime() - testDuck) < 0.05)
			return;
		#endif
	}
	else //set time duck was pushed
	{
		#ifdef CLIENT_DLL
		lastUnDuck = pm_time;
		#else
		testDuck = pmove->Sys_FloatTime();
		#endif
	}*/
...
//    qboolean canbunnyhop = false;
	...
	//if in mid jump // or falling oddly enough
	if ( pmove->oldbuttons & IN_JUMP )
	{	
		float jump_time = JUMP_TIME;
		#ifdef CLIENT_DLL //client timer for prediction
		//if jump is being pressed AND time is up // probably dont need the first part
		//if((pmove->cmd.buttons & IN_JUMP) && (pm_time - startTime >= 0.25))
		if(pm_time - startTime >= jump_time)
		{
			return;		// don't pogo stick
		}
		#else //server timer that matters
		//if jump is being pressed AND time is up // probably dont need the first part
		//if((pmove->cmd.buttons & IN_JUMP) && (pmove->Sys_FloatTime() - pmove->fuser4 >= 0.25))
		if(pmove->Sys_FloatTime() - pmove->fuser4 >= jump_time)
		{
			return;		// don't pogo stick
		}
		#endif
