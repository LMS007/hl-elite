...
CMessageWindowPanel::CMessageWindowPanel( const char *szMOTD, const char *szTitle, int iShadeFullscreen, int iRemoveMe, int x, int y, int wide, int tall ) : CMenuPanel( iShadeFullscreen ? 100 : 255, iRemoveMe, x, y, wide, tall )
{
	...
	//CTransparentPanel( 200, XRES(80), YRES(80), XRES(480), YRES(330));

	// Create the window
	m_pBackgroundPanel = new CTransparentPanel( /*iShadeFullscreen ? 255 : 100*/0, MOTD_WINDOW_X, MOTD_WINDOW_Y, MOTD_WINDOW_SIZE_X, MOTD_WINDOW_SIZE_Y );
	m_pBackgroundPanel->setParent( this );
	//m_pBackgroundPanel->setBorder( new LineBorder( Color(255 * 0.7,170 * 0.7,0,0)) );
	//m_pBackgroundPanel->setBorder( new LineBorder( Color(HLE_RED, HLE_GREEN,HLE_BLUE,HLE_ALPHA)) ); 
	m_pBackgroundPanel->setBorder( new LineBorder( Color(0, 0,0,0)) ); 
	...
	// shade in the backround
	m_pBackround = new Label( "", 0, 0, m_pBackgroundPanel->getWide(), m_pBackgroundPanel->getTall() );
	m_pBackround->setParent( m_pBackgroundPanel );
    m_pBackround->setBgColor( 0, 0, 0, 150);
	...
	// hle
	pButton->setHighLightColor( COMMAND_BUTTON_HIGHLIGHT_RED, COMMAND_BUTTON_HIGHLIGHT_GREEN, COMMAND_BUTTON_HIGHLIGHT_BLUE, COMMAND_BUTTON_HIGHLIGHT_ALPHA );
	pButton->setUnHighLightColor( 30, 30, 40, 100 );
