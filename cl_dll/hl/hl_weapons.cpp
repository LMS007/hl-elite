...
/*static*/ CBasePlayer	player; // major fucking hack 007
...
=====================
*/
Vector CBaseEntity::FireBulletsPlayer ( ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker, int shared_rand, float penetration )
{
	float x, y, z;
...
=====================
*/
void CBasePlayerWeapon::PrintState( BOOL missed )
{

	if (missed) {
		COM_Log( "c:\\hle_log\\hl.log", "MISS FIRE>>>>>>");
	}
	//COM_Log( "c:\\hle_log\\hl.log", "m_ellaspsedTime: %.6f, ", m_ellaspsedTime );
	COM_Log( "c:\\hle_log\\hl.log", "gpGlobals->time: %.4f\n", gpGlobals->time );
	COM_Log( "c:\\hle_log\\hl.log", "m_flNextPrimaryAttack: %.4f, ", m_flNextPrimaryAttack);
	COM_Log( "c:\\hle_log\\hl.log", "m_flPrevPrimaryAttack: %.4f, ", m_flPrevPrimaryAttack);
	COM_Log( "c:\\hle_log\\hl.log", "m_flLastPrimaryFireTime: %.4f\n", m_flLastPrimaryFireTime);
	COM_Log( "c:\\hle_log\\hl.log", "m_flNextSecondaryAttack: %.4f, ", m_flNextSecondaryAttack);
	COM_Log( "c:\\hle_log\\hl.log", "m_flPrevSecondaryAttack: %.4f, ", m_flPrevSecondaryAttack);
	COM_Log( "c:\\hle_log\\hl.log", "m_flLastSecondaryFireTime: %.4f\n", m_flLastSecondaryFireTime);
	COM_Log( "c:\\hle_log\\hl.log", "m_iClip: %d\n\n", m_iClip);

	
	/*COM_Log( "c:\\hle_log\\hl.log", "gpGlobals->time: %.6f, ", gpGlobals->time );
	COM_Log( "c:\\hle_log\\hl.log", "m_lastFireTime: %.6f, ", m_lastFireTime );
	COM_Log( "c:\\hle_log\\hl.log", "m_timeBetweenFires: %.6f, ", m_timeBetweenFires );
	COM_Log( "c:\\hle_log\\hl.log", "m_creep: %.6f, ", m_creep );
	COM_Log( "c:\\hle_log\\hl.log", "m_totalCreep: %.6f, ", m_totalCreep );
	//COM_Log( "c:\\hle_log\\hl.log", "m_delta: %.4f, ", m_delta );
	//COM_Log( "c:\\hle_log\\hl.log", "m_pPlayer->m_flNextAttack: %.4f, ", m_pPlayer->m_flNextAttack );
	
	COM_Log( "c:\\hle_log\\hl.log", ": m_flPrevPrimaryAttack %.6f, ", m_flPrevPrimaryAttack );
	COM_Log( "c:\\hle_log\\hl.log", ": m_flNextPrimaryAttack %.6f, ", m_flNextPrimaryAttack );
	//COM_Log( "c:\\hle_log\\hl.log", "m_flTimeWeaponIdle - gpGlobals->time: %.4f ", m_flTimeWeaponIdle - gpGlobals->time);
	COM_Log( "c:\\hle_log\\hl.log", "m_iClientClip: %i, ", m_iClientClip );
	COM_Log( "c:\\hle_log\\hl.log", "m_iClip: %i\n\n", m_iClip );
	*/
}
...
//		pCurrent->m_flPumpTime			= pfrom->m_flPumpTime;
	...
	player.ammo_556			= (int)from->client.fuser1;
	//player.ammo_shockbeams	= (int)from->client.fuser4;
	//player.ammo_spores		= (int)from->client.iuser4;
	...
	//hle reload fix
	to->client.fuser1					= player.ammo_556;
