...
extern int g_TeamOrder[MAX_TEAMS];
...
#define SBOARD_INDENT_X			XRES(90)//XRES(104)
#define SBOARD_INDENT_Y			YRES(40)

// low-res scoreboard indents
#define SBOARD_INDENT_X_512		30//30
...
int iTeamColors[8][3]=
{
	{ 150, 185, 225 },	// HLE (default) light blue
	{ 60, 110, 255 },	// Blue
	{ 255, 50, 10 },	// Red
	{ 50, 255, 50 },	// Green
	{ 255, 255, 45 },	// Yellow
...
void CCommandMenu::AddButton( CommandButton *pButton )
{
	...
	pButton->setBorder( new LineBorder( Color(140,172,225,0)));
...
CCommandMenu *TeamFortressViewport::CreateSubMenu( CommandButton *pButton, CCommandMenu *pParentMenu, int iYOffset, int iXOffset )
{
	...
	pButton->setBorder( new LineBorder( Color(140,172,225,0))); // hle
...
TeamFortressViewport::TeamFortressViewport(int x,int y,int wide,int tall) : Panel(x,y,wide,tall), m_SchemeManager(wide,tall)
{
	...
	m_pTeamMenu = NULL;
//	m_pClassMenu = NULL;
	m_pMainMenu = NULL; //hle 
	m_pVModesMenu = NULL; //hle
	m_pVSettingsMenu = NULL; //hle
	m_pVMatchMenu = NULL;
	m_pVMapsMenu	= NULL;
	m_pClientMenu = NULL;
    m_pVSpeedMenu = NULL;
...
//	CreateMiniMenu(); 
	...
	m_SpectatorOptionsMenu = CreateCommandMenu("spectatormenu.txt", 1, YRES(344), true, CMENU_SIZE_X, BUTTON_SIZE_Y / 2, 0 );	// above bottom bar, flat design
	m_SpectatorCameraMenu = CreateCommandMenu("spectcammenu.txt", 1, YRES(344), true, XRES( 200 ), BUTTON_SIZE_Y / 2, ScreenWidth - ( XRES ( 200 ) + 15 ) );	// above bottom bar, flat design
	
	CreateServerBrowser();

	//ClientCmd("cl_gausscolor.value"); // [ hle ] out of place
	if(!g_MapsDownloaded.m_bGotMaps)
	{
		ServerCmd("sendmemaps\n");
	}

	m_bTossInput = false;

	m_DropMessage.effect=2;
	m_DropMessage.r1=255;
	m_DropMessage.r2=255;	

	m_DropMessage.g1=255;
	m_DropMessage.g2=255;

	m_DropMessage.b1=255;
	m_DropMessage.b2=255;

	m_DropMessage.a1=0;
	m_DropMessage.a2=0;

	m_DropMessage.x=0.05f;
	m_DropMessage.y=0.6f;

	m_DropMessage.effect=0;
	m_DropMessage.fxtime=0.5;
	m_DropMessage.fadeout=2;	
	m_DropMessage.fadein=0.15;
	
	sprintf(m_szDropMessage, "Drop:\n\"H\" Health\n\"B\" Battery\n\"F\" Flag\n\"J\" Longjump\n\"N\" Weapon\n\"1\" Primary ammo\n\"2\" Secondary ammo\n\"SPACE\" Quit");
	m_DropMessage.pMessage=m_szDropMessage;
...
//	}
	...
	/*if (m_pVSettingsMenu)// hle
	{
		m_pVSettingsMenu->setVisible( false );
	}*/
	if (m_pVMatchMenu)// hle
	{
		m_pVMatchMenu->setVisible( false );
	}
    if (m_pVSpeedMenu)// hle
	{
		m_pVSpeedMenu->setVisible( false );
	}
	if (m_pVModesMenu)// hle
	{
		m_pMainMenu->setVisible( false );
	}
	if (m_pVMapsMenu)// hle
	{
		m_pVMapsMenu->setVisible( false );
	}
	if(m_pClientMenu)
	{
		m_pClientMenu->setVisible( false );
	}
	if (m_pScoreBoard)
	{
...
void TeamFortressViewport::UpdateSpectatorPanel()
{
	...
	if ( g_iUser1 && gHUD.m_pCvarDraw->value && !gHUD.m_iIntermission)	// don't draw in dev_overview mode
	{
		...
		{	// restore GUI color
			m_pSpectatorPanel->m_BottomMainLabel->setFgColor( HLE_RED, HLE_GREEN, HLE_BLUE, 0 );
		}
...
void TeamFortressViewport::CreateScoreBoard( void )
{
	...
	m_pScoreBoard->setVisible(false);
	m_pScoreBoard->setBorder( new LineBorder( Color(0,0,0,0)));
	m_pScoreBoard->setBgColor( 0, 0, 0, 100 );
}
...
void TeamFortressViewport::ShowVGUIMenu( int iMenu )
{
	...
	if ( gHUD.m_iIntermission && (iMenu != MENU_INTRO && iMenu != MENU_TEAM) ) // hle change "MEUN_TEAM" added here
		return;
...
//	case MENU_SETTINGS:
//		pNewMenu = ShowVSettingsMenu();
//		break;

	case MENU_MATCH:
		pNewMenu = ShowVMatchMenu();
		break;

	case MENU_MODES:
		pNewMenu = ShowVModesMenu();
		break;

	case MENU_MAPS:
        
		pNewMenu = ShowVMapsMenu();
		break;
    case MENU_SPEED:
		pNewMenu = ShowVSpeedMenu();
		break;
	case MENU_CLIENT:
		pNewMenu = ShowClientMenu();
		break;
	case MENU_HELP:
		pNewMenu = ShowHelpMenu();
	default:
		break;
	}
...
void TeamFortressViewport::CreateHelpMenu()
{
	// Create the panel
	m_pHelpMenu = new CHelpMenu(255, false, 0, 0, ScreenWidth, ScreenHeight);
	m_pHelpMenu->setParent( this );
	m_pHelpMenu->setVisible( false );
}
...
void TeamFortressViewport::CreateMainMenu()
{
	...
	// Create the panel
	m_pMainMenu = new CMainMenu(255, false, 0, 0, ScreenWidth, ScreenHeight);
	m_pMainMenu->setParent(this);
	m_pMainMenu->setVisible( false );
}
/*
void TeamFortressViewport::CreateVSettingsMenu()
{
	// Create the panel
	m_pVSettingsMenu = new CVoteSettingsMenu(255, false, 0, 0, ScreenWidth, ScreenHeight);
	m_pVSettingsMenu->setParent(this);
	m_pVSettingsMenu->setVisible( false );
}*/

void TeamFortressViewport::CreateVMatchMenu()
{
	// Create the panel
	m_pVMatchMenu = new CVoteMatchMenu(255, false, 0, 0, ScreenWidth, ScreenHeight);
	m_pVMatchMenu->setParent(this);
	m_pVMatchMenu->setVisible( false );
}
void TeamFortressViewport::CreateVMapsMenu()
{
//    AllocMapData("
	// Create the panel
	m_pVMapsMenu = new CVoteMapsMenu(255, false, 0, 0, ScreenWidth, ScreenHeight);
	m_pVMapsMenu->setParent(this);
	m_pVMapsMenu->setVisible( false );
}

void TeamFortressViewport::CreateVModesMenu()
{
	// Create the panel
	m_pVModesMenu = new CVoteModesMenu(255, false, 0, 0, ScreenWidth, ScreenHeight);
	m_pVModesMenu->setParent(this);
	m_pVModesMenu->setVisible( false );
}

void TeamFortressViewport::CreateClientMenu()
{
	// Create the panel
	m_pClientMenu = new CClientMenu(255, false, 0, 0, ScreenWidth, ScreenHeight);
	m_pClientMenu->setParent(this);
	m_pClientMenu->setVisible( false );
}
...
{
	// Create the panel
	m_pMp3 = new CMp3Player(255, false, 0, 0, ScreenWidth, ScreenHeight);
	m_pMp3->setParent(this);
	m_pMp3->setVisible( false );
}
...
void TeamFortressViewport::CreateVSpeedMenu()
{
	// Create the panel
	m_pVSpeedMenu = new CVoteSpeedMenu(255, false, 0, 0, ScreenWidth, ScreenHeight);
	m_pVSpeedMenu->setParent(this);
	m_pVSpeedMenu->setVisible( false );
}


void TeamFortressViewport::CreateMiniMenu()
{
//	m_pMiniMenu = new CMiniMenu();
//	m_pMiniMenu->setParent(this);
//	m_pMiniMenu->setVisible( true );
}
/*
void TeamFortressViewport::CreateRadar()
{
	m_pRadar = new CRadar(XRES(2), YRES(2),XRES(80), YRES(80));
	m_pRadar->setParent(this);
	m_pRadar->setVisible(false);
}*/
...
void TeamFortressViewport::ShowMiniMenu()
{
//	if (m_pMiniMenu)
	{
//		m_pMiniMenu->setVisible(true);	
...
CMenuPanel* TeamFortressViewport::ShowMainMenu()
{
	...
	m_pMainMenu->Reset();
	m_pMainMenu->ResetButtons();
	return m_pMainMenu;
}

/*
void TeamFortressViewport::ShowRadar()
{
	if (!gEngfuncs.pDemoAPI->IsPlayingback() )
	{
		if(cl_radar!=0)
			m_pRadar->setVisible(true);
	}
}
*/
/*
void TeamFortressViewport::HideRadar()
{
	m_pRadar->setVisible(false);
}
*/
...
CMenuPanel* TeamFortressViewport::ShowVMapsMenu()
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;

    if(!m_pVMapsMenu->m_bInitedButtons)
    {
		
		m_pVMapsMenu->m_bInitedButtons = true;
		m_pVMapsMenu->InItMapsButtons();  
    }

	m_pVMapsMenu->Reset();
	m_pVMapsMenu->ResetButtons();
	return m_pVMapsMenu;
}

CMenuPanel* TeamFortressViewport::ShowVSpeedMenu()
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;
	m_pVSpeedMenu->Reset();
	m_pVSpeedMenu->ResetButtons();
	return m_pVSpeedMenu;
}

/*
CMenuPanel* TeamFortressViewport::ShowVSettingsMenu()
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;
	m_pVSettingsMenu->Reset();
	m_pVSettingsMenu->ResetButtons();
	return m_pVSettingsMenu;
}
*/

CMenuPanel* TeamFortressViewport::ShowVMatchMenu()
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;
	m_pVMatchMenu->Reset();
	m_pVMatchMenu->ResetButtons();
	return m_pVMatchMenu;
}

CMenuPanel* TeamFortressViewport::ShowVModesMenu()
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;
	m_pVModesMenu->Reset();
	m_pVModesMenu->ResetButtons();
	return m_pVModesMenu;
}

CMenuPanel* TeamFortressViewport::ShowClientMenu()
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;
	...
	m_pClientMenu->Reset();
	m_pClientMenu->ResetButtons();
	return m_pClientMenu;
}
CMenuPanel* TeamFortressViewport::ShowHelpMenu()
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;
...
void TeamFortressViewport::paintBackground()
{
	...
	//gHUD.m_TextMessage.MsgFunc_TextMsg( NULL, strlen( b ) + 1, b );
	if (m_pScoreBoard)
	{
		int x, y;
...
int	TeamFortressViewport::KeyInput( int down, int keynum, const char *pszCurrentBinding )
{
	...
	{
		//if ( down && (keynum == K_ENTER || keynum == K_KP_ENTER) ) HLE EDIT out
	...
	if (m_pCurrentMenu && gEngfuncs.Con_IsVisible() == false)
	{
		...
		/*if ( pszCurrentBinding && down && !strcmp(pszCurrentBinding, "+jump") )
		{
			...
			{
				m_pTeamMenu->SlotInput(7);
				return 0;
			}
		}*/  // HLE comment this block out
	...
	if(m_bTossInput /*&& gViewPort->m_iDropDelay < gHUD.m_flTime*/)
	{	
		if ( keynum == 'b' || keynum == 'B')
		{
			m_bTossInput = false;
			m_DropMessage.holdtime=0;
			m_DropMessage.fadeout=0;
			ServerCmd("drop_battery");
			return 0;
		}
		else if ( keynum == 'h' || keynum == 'H')
		{
			m_bTossInput = false;
			m_DropMessage.holdtime=0;
			m_DropMessage.fadeout=0;
			ServerCmd("drop_health");
			return 0;
		}
		else if ( keynum == 'j' || keynum == 'J')
		{
			m_bTossInput = false;
			m_DropMessage.holdtime=0;
			m_DropMessage.fadeout=0;
			ServerCmd("drop_longjump");
			return 0;
		}
		else if ( keynum == 'n' || keynum == 'N')
		{
			m_bTossInput = false;
			m_DropMessage.holdtime=0;
			m_DropMessage.fadeout=0;
			ServerCmd("drop_weapon");
			return 0;
		}
		else if ( keynum == '1')
		{
		...
		else if ( keynum == '2')
		{
			...
			m_DropMessage.holdtime=0;
			m_DropMessage.fadeout=0;
			ServerCmd("drop_ammo2");
			return 0;
		}
		else if ( keynum == 'f' || keynum == 'F')
		{
			m_bTossInput = false;
			m_DropMessage.holdtime=0;
			m_DropMessage.fadeout=0;
...
int TeamFortressViewport::MsgFunc_IconInfo( const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );
	int cl			= READ_BYTE();
	int teamnumber	= READ_BYTE();
	int spectator	= READ_BYTE();
	int removed		= READ_BYTE();
	int flag			= READ_BYTE();
	if ( cl > 0 && cl <= MAX_PLAYERS )
	{
		g_PlayerExtraInfo[cl].spectator = spectator;
		if(removed)
			g_PlayerExtraInfo[cl].aux2 = 2; // RED X TGA
		else if(spectator)
		{
			g_PlayerExtraInfo[cl].aux2 = 1; // EYES TGA
		}
		else if(flag)
		{
			g_PlayerExtraInfo[cl].aux2 = 3;	// FLAG TGA
			g_PlayerExtraInfo[cl].flag_color = flag;
		}
		else
			g_PlayerExtraInfo[cl].aux2 = 0; // NO IMAGE
		
	/*	if(g_TeamplayClient && teamnumber==0)
		{
			g_PlayerExtraInfo[cl].teamnumber = 7;
		}
		else*/
...
int TeamFortressViewport::MsgFunc_ScoreInfo( const char *pszName, int iSize, void *pbuf )
{
	...
	short aux0			= READ_SHORT(); // hle
	short aux1			= READ_SHORT();// hle
	//short aux2			= READ_BYTE();
	...
	if ( cl > 0 && cl <= MAX_PLAYERS )
	{
		...
		//g_PlayerExtraInfo[cl].teamnumber = teamnumber;
		g_PlayerExtraInfo[cl].aux0 = aux0;
		g_PlayerExtraInfo[cl].aux1 = aux1;
...
// accepts three values plus 2 auxillery values for game modes with extra hud info
//		string: team name
//		short: teams kills
//		short: teams deaths 
...
int TeamFortressViewport::MsgFunc_TeamScore( const char *pszName, int iSize, void *pbuf )
{
	...
	for ( int i = 1; i <= m_pScoreBoard->m_pColumnInfo->m_iNumTeams; i++ )
	{
		if ( !stricmp( TeamName, g_TeamInfo[i].name ) )
			break;
	}
	int i;
	if ( i > m_pScoreBoard->m_pColumnInfo->m_iNumTeams )
		return 1;
	...
	g_TeamInfo[i].aux0		= READ_SHORT();
	g_TeamInfo[i].aux1		= READ_SHORT();
	return 1;
}
...
int TeamFortressViewport::MsgFunc_TeamOrder( const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );
	
	int team  = READ_BYTE()+1;
	int order = READ_BYTE()+1;
	g_TeamOrder[team]=order;
	return 1;
}
...
int TeamFortressViewport::MsgFunc_PlayerID( const char *pszName, int iSize, void *pbuf )
{
	if (!m_pScoreBoard)
		return 1;
	...
	{  
		// set the players team
		sprintf(g_PlayerExtraInfo[cl].playerID, "%s", READ_STRING());
	}
	// rebuild the list of teams
	//m_pScoreBoard->RebuildTeams();
