...
class CMainMenu; // hle
class CVoteSettingsMenu; // hle
class CVoteMatchMenu;
class CVoteSpeedMenu;
class CVoteModesMenu;
class CVoteMapsMenu;

class CVoteOptionsMenu;
class CVoteTimeMenu;
class CVoteFragMenu;
class CVoteWinMenu;
class CMiniMenu;
...
extern int iTeamColors[8][3];
extern int iFlagColors[8][3];
extern int iNumberOfTeamColors;
extern int g_nMapsOnServer;


extern int g_nHighFrags;
extern int g_nHighWins;
extern int g_TeamOrder[MAX_TEAMS];

#define PLAYERS_PER_PAGE 15
...
#define HLE_RED 170
#define HLE_GREEN 210
#define HLE_BLUE 255
#define HLE_ALPHA 0

#define COMMAND_BUTTON_HIGHLIGHT_RED 220
#define COMMAND_BUTTON_HIGHLIGHT_GREEN 230
#define COMMAND_BUTTON_HIGHLIGHT_BLUE 255
#define COMMAND_BUTTON_HIGHLIGHT_ALPHA 160

#define COMMAND_BUTTON_RED 150
#define COMMAND_BUTTON_GREEN 185
#define COMMAND_BUTTON_BLUE 225
#define COMMAND_BUTTON_ALPHA 200

#define COMMAND_BUTTON_DISABLED 150
#define COMMAND_BUTTON_DISABLED_ALPHA 225
/*
#define COMMAND_BUTTON_RED 90
#define COMMAND_BUTTON_GREEN 136
#define COMMAND_BUTTON_BLUE 196
#define COMMAND_BUTTON_ALPHA 50
*/
...
private:
	...
	int high_r, high_b, high_g, high_a;
	int unhigh_r, unhigh_b, unhigh_g, unhigh_a;

	char high_r_text, high_b_text, high_g_text, high_a_text;
	char unhigh_r_text, unhigh_b_text, unhigh_g_text, unhigh_a_text;
...
public:
	...
	void SetHightLightTextColor(int r, int g, int b, int a)
	{
		high_r_text = r;
		high_g_text = g;
		high_b_text = b;
	...
	void SetUnHightLightTextColor(int r, int g, int b, int a)
	{
		unhigh_r_text = r;
		unhigh_g_text = g;
		unhigh_b_text = b;
	...
	void getHighLightColor(int* r, int* g, int* b, int* a)
	{
		*r = high_r;
		*b = high_b;
		*g = high_g;
	...
	void getUnHighLightColor(int* r, int* g, int* b, int* a)
	{
		*r = unhigh_r;
		*b = unhigh_b;
		*g = unhigh_g;
	...
	void setUnHighLightColor(int r, int g, int b, int a)
	{
		unhigh_r = r;
		unhigh_b = b;
		unhigh_g = g;
...
public:
	...
	void paint()
	{
		...
		else
		{
			...
			{
				UnArmedColor->getColor(r, g, b, a);
				setFgColor(HLE_RED, HLE_GREEN, HLE_BLUE, HLE_ALPHA);
			}
			//else
...
class HLEMenuButton : public CommandButton
{
	public:
	HLEMenuButton(Panel* parent, const char* text,int x,int y,int wide,int tall );
	/*{
		//Init();
...
class ExitButton : public HLEMenuButton
{
	public:
		ExitButton::ExitButton(Panel* parent, int x,int y,int wide,int tall) : HLEMenuButton(parent, "", x, y, wide, tall)
		{
		}
		virtual void paintBackground()
		{
...
public:
	...
	virtual void paintBackground()
	{
		if ( isArmed())
		{
			...
			drawSetColor(COMMAND_BUTTON_HIGHLIGHT_RED,
				COMMAND_BUTTON_HIGHLIGHT_GREEN, 
				COMMAND_BUTTON_HIGHLIGHT_BLUE,
				COMMAND_BUTTON_HIGHLIGHT_ALPHA );
	...
	virtual void paint()
	{
		...
		if ( isArmed() )
		{
			...
			setFgColor( 255, 255, 255, HLE_ALPHA ); 
		}
		else
		{
			//setFgColor( 143, 143, 54, 15 ); HLE
			setFgColor( HLE_RED, HLE_GREEN, HLE_BLUE, HLE_ALPHA ); 
		}
...
//	CMenuPanel*	 ShowClassMenu( void );
	...
	void		 CreateMainMenu( void );//hle
	CMenuPanel*	 ShowMainMenu( void );//hle
	
	//void		 CreateVSettingsMenu( void );//hle
	//CMenuPanel*	 ShowVSettingsMenu( void );//hle

	void		 CreateVMatchMenu( void );//hle
	CMenuPanel*	 ShowVMatchMenu( void );//hle

	void		 CreateVModesMenu( void );//hle
	CMenuPanel*	 ShowVModesMenu( void );//hle

	void		 CreateVMapsMenu( void );//hle
	CMenuPanel*	 ShowVMapsMenu( void );//hle

	void		CreateRadar();
//
    void		 CreateVOptionsMenu( void );//hle
	CMenuPanel*	 ShowVOptionsMenu( void );//hle

	void		CreateClientMenu( void );
	CMenuPanel* ShowClientMenu( void );

    void		 CreateVSpeedMenu( void );//hle
	CMenuPanel*	 ShowVSpeedMenu( void );//hle

	void		CreateHelpMenu(void);
	CMenuPanel*	ShowHelpMenu(void);
	...
	CommandButton *m_pTeamButtons[7];
	CommandButton *m_pDisguiseButtons[7];
	...
	char		m_sTeamNames[7][MAX_TEAMNAME_SIZE];
...
public:
	...
	bool		m_bTossInput;
	client_textmessage_t m_DropMessage;
	char		m_szDropMessage[1024];
...
//	void ShowRadar( void ); //hle
//	void HideRadar(void);

	void HideScoreBoard( void );
	bool IsScoreBoardVisible( void );

	bool IsMiniMenuVisible( void ); // hle
	void ShowMiniMenu( void ); // hle
	...
	char *GetTeamName( int iTeam ) { return m_sTeamNames[iTeam]; }; // hle team order crap
	...
	int	MsgFunc_IconInfo( const char *pszName, int iSize, void *pbuf );
	int MsgFunc_TeamScore( const char *pszName, int iSize, void *pbuf );
	int MsgFunc_TeamInfo( const char *pszName, int iSize, void *pbuf );
	int MsgFunc_TeamOrder( const char *pszName, int iSize, void *pbuf );
//	int MsgFunc_Spectator( const char *pszName, int iSize, void *pbuf );
	...
	int MsgFunc_Toss( const char *pszName, int iSize, void *pbuf );
...
public:
	...
	CMainMenu		*m_pMainMenu; //hle main
	CVoteSettingsMenu	*m_pVSettingsMenu; //hle sub
	CVoteMatchMenu	*m_pVMatchMenu; //hle sub
	CVoteModesMenu  *m_pVModesMenu;
	CClientMenu	*m_pClientMenu;
    CVoteSpeedMenu  *m_pVSpeedMenu;
	CVoteMapsMenu	*m_pVMapsMenu;
    CVoteOptionsMenu *m_pVOptionsMenu;
	CHelpMenu		*m_pHelpMenu;
	CMp3Player		*m_pMp3;
//    CVoteFragMenu   *m_pVFragMenu;
//    CVoteTimeMenu   *m_pVTimeMenu;
//    CVoteWinMenu    *m_pVWinMenu;
	ScorePanel		*m_pScoreBoard;
	CMiniMenu		*m_pMiniMenu;
//	CRadar			*m_pRadar;
...
// this is compete crap, change this later ( hle )
class CMenuHandler_ChangeTeam : public CMenuHandler_StringCommand
{
public:
	CMenuHandler_ChangeTeam( char *pszCommand ) : CMenuHandler_StringCommand( pszCommand )
	{
	}

	CMenuHandler_ChangeTeam( char *pszCommand, int iClose ) : CMenuHandler_StringCommand( pszCommand, iClose )
	{
	}
	...
	virtual void actionPerformed(Panel* panel)
	{
		...
		*buf = NULL;
		...
		strcat(buf, "changeteam ");

		int nTeam = atoi(m_pszCommand);

		if(nTeam >0 && nTeam <= gViewPort->m_iNumberOfTeams)
		{
			strcat(buf,gViewPort->GetTeamName(nTeam));
...
class CMenuHandler_SoundCommand : public ActionSignal
{
protected:
	char m_cSound[255];
...
public:
	...
	CMenuHandler_SoundCommand( char *pszCommand )
	{
		*m_cSound = NULL;
		strcpy(m_cSound, pszCommand);
	}

	virtual void actionPerformed(Panel* panel)
	{
		//gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_STATIC, EGON_SOUND_RUN, 0.98, ATTN_NORM, 0, 125 );
		PlaySound(m_cSound, 1);
	}
};
...
public:
	...
	virtual void paintBackground()
	{
		if ( isArmed() )
		{
			//drawSetColor( 143,143, 54, 125 );  hle
			drawSetColor( COMMAND_BUTTON_HIGHLIGHT_RED,COMMAND_BUTTON_HIGHLIGHT_GREEN, COMMAND_BUTTON_HIGHLIGHT_BLUE, COMMAND_BUTTON_HIGHLIGHT_ALPHA ); 
			drawFilledRect( 0, 0,_size[0] - 1,_size[1]);
		}
	}
...
public:
	...
	Panel				*m_pMapWindow;
	Label				*m_pLabel;
	Label				*m_pPictureLabel;
	
	CommandButton		*m_pButtons[6];
	CommandButton		*m_pCancelButton;
	CommandButton		*m_pSpectateButton;
	CommandButton		*m_pMainMenu;
	int					m_iCurrentInfo;

public:
	
	BitmapTGA			*m_pTeamPics[6];
