...
void CHudSayText :: SayTextPrint( const char *pszBuf, int iBufSize, int clientIndex )
{
	...
	if ( *pszBuf == 2 && clientIndex > 0 )
	{
		...
		///
		cl_entity_t	*player = gEngfuncs.GetEntityByIndex(clientIndex); 
		...
		*tempBuf=NULL;
		...
		for( h = 0; h+k < 255 && h < iBufSize; h++)
		{
			if(pszBuf[h] == '%')
			{
				if(h+1<255)
				{
					if(pszBuf[h+1] == 'l' || pszBuf[h+1] == 'L') // relplace %l with the locatin string 
					{
						char* szLoc = gHUD.m_Locs.GetClosestLoc(&player->origin);
						int len = strlen(szLoc);
						int j;
						for(j = 0; j < len && j < 255; j++)
						{
							tempBuf[h+j+k] = szLoc[j];
						}
						k += j-2;
						h++;
						continue;						
			...
			tempBuf[h+k] = pszBuf[h];
		}///
		iBufSize = strlen(tempBuf)+1;

		if ( pName )
		{
			const char *nameInString = strstr( tempBuf, pName );

			if ( nameInString )
			{
				g_iNameLengths[i] = strlen( pName ) + (nameInString - tempBuf);
				g_pflNameColors[i] = GetClientColor( clientIndex );
			}
		}
	}

	strncpy( g_szLineBuffer[i], tempBuf, max(iBufSize -1, MAX_CHARS_PER_LINE-1) );
