...
void CommandButton::Init( void )
{
	...
	unhigh_r_text = (char)150;
	unhigh_g_text = (char)185; 
	unhigh_b_text = (char)225; 
	unhigh_a_text = (char)0;

	high_r_text = (char)255;
	high_g_text = (char)255; 
	high_b_text = (char)255; 
	high_a_text = (char)0;
	
	setFgColor(unhigh_r_text, unhigh_g_text, unhigh_b_text, unhigh_a_text); //hle
...
void CommandButton::setActive( bool bActive )
{
	m_bDeactivated = !bActive;
}
...
void CommandButton::setDisabled(bool disabled)
{	
	// hard code the colors, i'm just getting lazy with this...
	if(disabled)
	{
		m_bDisabled = true;
		setHighLightColor( COMMAND_BUTTON_RED, COMMAND_BUTTON_GREEN, COMMAND_BUTTON_BLUE, COMMAND_BUTTON_DISABLED_ALPHA );
		setUnHighLightColor( COMMAND_BUTTON_RED, COMMAND_BUTTON_GREEN, COMMAND_BUTTON_BLUE, COMMAND_BUTTON_DISABLED_ALPHA );
		SetUnHightLightTextColor(255,255,255,225);
		SetHightLightTextColor(255,255,255,225);
...
void CommandButton::paint()
{
	...
	if ( isArmed() )
	{
		...
		setFgColor(high_r_text, high_g_text, high_b_text, high_a_text); //hle
	}
	else
	{
		//setFgColor( Scheme::sc_primary1 );
...
void CommandButton::paintBackground()
{
	if ( isArmed() )
	{
		drawSetColor(high_r, high_g, high_b, high_a );
	...
	{
		drawSetColor(unhigh_r, unhigh_g, unhigh_b, unhigh_a );
		drawFilledRect(0,0,_size[0],_size[1]);
	}
	...
	drawFilledRect(0,0,1,_size[1]);
	drawFilledRect(0,0,_size[0],1);

    /////////////// BLACK 
    drawSetColor(0,0,0,100);
	drawOutlinedRect(_size[0]-1,0,_size[0]+1,_size[1]+1);
	drawOutlinedRect(0,_size[1]-1,_size[0]+1,_size[1]+1);
...
void CHandler_MenuButtonOver::cursorExited(Panel* panel) 
{
	// hle
	if ( gViewPort && m_pMenuPanel )
	{
		m_pMenuPanel->SetActiveInfo( -1 );
	}//
}
