...
//	p->iWeight = m_pPlayer->m_nWeight[WEAPON_HANDGRENADE];
...
void CHandGrenade::WeaponIdle( void )
{
	...
	if ( m_flStartThrow )
	{
		...
		Vector vecSrc = m_pPlayer->pev->origin + m_pPlayer->pev->view_ofs + gpGlobals->v_forward * 8;
		...
		SetNextPrimaryAttackDelay(0.35);
		...
		if ( !m_pPlayer->m_rgAmmo[ m_iPrimaryAmmoType ] )
		{
			...
			SetNextPrimaryAttackDelay(0.35);
			SetNextSecondaryAttackDelay(0.35);
			m_flTimeWeaponIdle = m_flNextPrimaryAttack; // ensure that the animation can finish playing
		}
		return;
	}
