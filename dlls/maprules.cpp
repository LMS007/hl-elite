...
class HLEGameScore : public CRulePointEntity
{
public:
	void	Spawn( void );
...
LINK_ENTITY_TO_CLASS( hle_game_score, HLEGameScore );


void HLEGameScore::Spawn( void )
{
	CRulePointEntity::Spawn();
}


void HLEGameScore::KeyValue( KeyValueData *pkvd )
{
...
void HLEGameScore::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
...
void CGameTeamMaster::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( useType == USE_SET )
	{
		...
		{
			m_teamIndex = g_pGameRules->GetTeamIndex2( pActivator->TeamID() );
		}
		return;
	}
