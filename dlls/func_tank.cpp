...
BOOL CFuncTank :: StartControl( CBasePlayer *pController )
{
	...
	// make crosshair message here! hle
...
void CFuncTankMortar::Fire( const Vector &barrelEnd, const Vector &forward, entvars_t *pevAttacker )
{
	if ( m_fireLast != 0 )
	{
		...
		if ( bulletCount > 0 )
		{
			...
			CBasePlayer *pPlayer = (CBasePlayer*) CBasePlayer::Instance( pevAttacker ); // mighjt need a player check hle

			ExplosionCreate( tr.vecEndPos, pev->angles, pPlayer->edict(), pev->impulse, TRUE );
