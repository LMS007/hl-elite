...
void CSqueakGrenade :: Spawn( void )
{
	...
	pev->dmg = 0;//gSkillData.snarkDmgPop;
...
LINK_ENTITY_TO_CLASS( weapon_penguin, CSqueak ); // new link penguin to snark - santagauss
LINK_ENTITY_TO_CLASS( weapon_snark, CSqueak );


void CSqueak::Spawn( )
{
	pev->classname = MAKE_STRING("weapon_snark"); // new - santagauss
...
//	p->iWeight = m_pPlayer->m_nWeight[WEAPON_SNARK];
...
void CSqueak::PrimaryAttack()
{
	...
	{
		if (FClassnameIs( pEntOut->pev, "monster_snark"))
		{
			CSqueakGrenade *pSnarkOut = (CSqueakGrenade*)pEntOut;

			if (pSnarkOut->m_hOwner.Get() == pPlayer)
			{
				iHowManyOut++;
			}
		}
	...
	if(iHowManyOut >= SNARK_MAX_CARRY)
		return;
#endif
	// END TOTAL TEST
