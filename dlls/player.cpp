...
TYPEDESCRIPTION	CBasePlayer::m_playerSaveData[] = 
{
    ...
    DEFINE_FIELD( CBasePlayer, m_isSpectator, FIELD_BOOLEAN ), // hle

	DEFINE_FIELD( CBasePlayer, m_pTank, FIELD_EHANDLE ),
	DEFINE_FIELD( CBasePlayer, m_iHideHUD, FIELD_INTEGER ),
	DEFINE_FIELD( CBasePlayer, m_iFOV, FIELD_INTEGER ),

	//DEFINE_FIELD( CBasePlayer, m_lObservers, FIELD_EDICT ),
...
int gmsgBattery = 0;
int gmsgTrain = 0;
//int gmsgLogo = 0; // REMOVED BY 007 bcaeuse we need message space
...
int gmsgSayText = 0;
int gmsgTextMsg = 0;
int gmsgSetFOV = 0; //check this one
int gmsgShowMenu = 0;
...
void LinkUserMessages( void )
{
	...
	gmsgFragsLeft = REG_USER_MSG( "FragsLeft", 2 );
	gmsgMenLeft = REG_USER_MSG( "MenLeft", 2 );
	gmsgVGUIMenu = REG_USER_MSG("VGUIMenu", 1); // hle
	gmsgCountNumber = REG_USER_MSG("CountNumber", 2);
    gmsgMapNames = REG_USER_MSG("MapNames", -1);
	gmsgBonus = REG_USER_MSG("Bonus", 1);
	gmsgCtfEvent = REG_USER_MSG("CtfEvent", 3);
//	gmsgCtfFlag =     REG_USER_MSG("CtfFlag", 1);
	gmsgReset =     REG_USER_MSG("Reset", 0);	
	//gmsgGaussColor=		REG_USER_MSG("GaussColor", 1);
	gmsgColor =		REG_USER_MSG("Color", 3);
	gmsgChangeXHair =	REG_USER_MSG("ChangeXHair", -1);
	gmsgStatusText = REG_USER_MSG("StatusText", -1);
	gmsgStatusValue = REG_USER_MSG("StatusValue", 3);
	gmsgTimer = REG_USER_MSG("Timer", 2);
//	gmsgWeight = REG_USER_MSG("Weight", 0);
	gmsgHudStatus = REG_USER_MSG("HudStatus", 1);
	gmsgFixEgon = REG_USER_MSG("FixEgon", 0);
	gmsgZoom = REG_USER_MSG("Zoom", 1);
	gmsgCtpEvent = REG_USER_MSG("CtpEvent", 7);
	gmsgCtpTime = REG_USER_MSG("CtpTime", 3);
	gmsgCap = REG_USER_MSG("Cap", 4);
	gmsgTeamOrder = REG_USER_MSG("TeamOrder", 2);
	gmsgRadar = REG_USER_MSG("Radar", 3);	
	gmsgIconInfo = REG_USER_MSG( "IconInfo", 5 );
	gmsgPlayerID = REG_USER_MSG("PlayerID", -1);
	gmsgTeamScores = REG_USER_MSG("TeamScores", 3);
	gmsgFlag		= REG_USER_MSG("Flag", 10);
	gmsgLocation	= REG_USER_MSG("Location", 7);
	gmsgAnounce	= REG_USER_MSG("Anounce", 3);
...
void CBasePlayer::PackDeadPlayerItems( void )
{
		...
		debug_file.OpenFile();
		debug_file << "Enter PackDeadPlayerItems\n";
 	...
 	if ( iWeaponRules == GR_PLR_DROP_GUN_NO && iAmmoRules == GR_PLR_DROP_AMMO_NO )
	{
		...
		// Find this players rocket launcher if they have one...
		for (int k = 0 ; k < MAX_ITEM_TYPES ; k++ )
		{
			if(m_rgpPlayerItems[k])
			{
				pItem = m_rgpPlayerItems[k];
				while (pItem)
				{						
					if(FStrEq(STRING(pItem->pev->classname), "weapon_rpg"))
					{
						goto FOUND_ROCKETLAUNCHER;
					}
					pItem = pItem->m_pNext;
				}			
			}
...
FOUND_ROCKETLAUNCHER:

		CBaseEntity* pEntity = UTIL_FindEntityByClassname( NULL, "rpg_rocket");
		while (pEntity)
		{
			if (pEntity->IsAlive())
			{
				CRpgRocket* pRocket = (CRpgRocket*)pEntity;
				if(pRocket->m_pLauncher)
				{
					if(pRocket->m_pLauncher == pItem)
					{
		...
		// rpg fix for action mode
		/*if(m_pActiveItem)
		{
			if(FStrEq(STRING(m_pActiveItem->pev->classname), "weapon_rpg"))
			{
				CRpgRocket* pRocket;
				...
				while (pEntity)
				{
					pRocket = (CRpgRocket*)pEntity;
					if(pRocket->m_pLauncher == m_pActiveItem)
					{
						pRocket->m_pLauncher=NULL;
						break;
					}
		...
		#ifdef HLE_DEBUG2 // DEBUG FILE DUMP ==============	
		debug_file.OpenFile();
		debug_file << "Exit PackDeadPlayerItems\n";
		debug_file.CloseFile();
...
void CBasePlayer::RemoveAllItems( BOOL removeSuit )
{
		...
		debug_file.OpenFile();
		debug_file << "Enter RemoveAllItems\n";
		debug_file.CloseFile();
	...
	if(m_fLongJump) 
	{
		CBaseEntity* pEntity = UTIL_FindEntityByClassname( NULL, "ljf");
		while (pEntity)
		{
			CLongJumpFollow* pLj = (CLongJumpFollow*)pEntity;
			if(pLj->m_pOwner == this)
			{
				pLj->Remove();
				m_fLongJump = 0;
				break;
			}
			pEntity = UTIL_FindEntityByClassname( pEntity, "ljf");
		}
	}
	if(g_teamplay) 
	{
		CBaseEntity* pEntity = UTIL_FindEntityByClassname( NULL, "_ti");
		while (pEntity)
		{
			CTeamIndicator* pId = (CTeamIndicator*)pEntity;
			if(pId->m_pOwner == this)
			{
				pId->Remove();
				break;
			}
			pEntity = UTIL_FindEntityByClassname( pEntity, "_ti");
		}
	}
	if(m_bHasFlag)
	{
	...
	MESSAGE_END();
		...
		debug_file.OpenFile();
		debug_file << "Exit RemoveAllItems\n";
		debug_file.CloseFile();
...
void CBasePlayer::Killed( entvars_t *pevAttacker, int iGib )
{
		...
		debug_file.OpenFile();
		debug_file << "Enter CBasePlayer::Killed\n";
		debug_file.CloseFile();
	...
	MESSAGE_END();
/*
	for(int k =0; k<m_lObservers.size(); k++)
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgSetFOV, NULL, m_lObservers[k] );
		WRITE_BYTE(0);
		MESSAGE_END();
	}*/
	...
	pev->nextthink = gpGlobals->time + 0.1;
		...
		debug_file.OpenFile();
		debug_file << "Exit CBasePlayer::Killed\n";
		debug_file.CloseFile();
...
void CBasePlayer::TabulateAmmo()
{
	...
	ammo_shockbeams = AmmoInventory( GetAmmoIndex( "ShockBeams" ) );
...
/*	if(m_bHasFlag)
	{
 	...
 	if(loselongjump.value !=0)
	{
		if(m_fLongJump)
		{
			CItemLongJump::Drop(this);
			m_fLongJump = 1; // hack to get the model to delete later on
		}
	}
	...
	if (pev->deadflag == DEAD_DEAD)
	{
		...
		if ( g_pGameRules->FPlayerCanRespawn( this ) )
		{
			...
			n_dDeathDelay.AddTime(g_pGameRules->DeathTime());
...
// choose to respawn.
	if ( g_pGameRules->IsMultiplayer() && (n_dDeathDelay.TimeUp() ) && !(m_afPhysicsFlags & PFLAG_OBSERVER) )
	{
		// go to dead camera. 
		if(forcerespawn.value==0)
			StartDeathCam(); // no buttons touched
	}
	
// wait for any button down,  or mp_forcerespawn is set and the respawn time is up
	if (!fAnyButtonDown 
		&& !( g_pGameRules->IsMultiplayer() && forcerespawn.value > 0 && /*(gpGlobals->time > (m_fDeadTime + 5))*/n_dDeathDelay.TimeUp()) )
		return;

	if(!m_isSpectator) // button touched
	{
		pev->button = 0;
		m_iRespawnFrames = 0;
...
//	int iRand;
	...
	CopyToBodyQue( pev );
	if(g_pGameRules->CanStartAutoObserver(this) && g_pGameRules->CanStartObserver(this))
	{
...
/*
	...
	if ( !FNullEnt( pSpot ) )
	{
		...
		CopyToBodyQue( pev );
		if(g_pGameRules->CanStartAutoObserver(this))
		{
			if(g_pGameRules->CanStartObserver(this))
			{
...
void CBasePlayer::Jump()
{
	...
	if ( !(pev->flags & FL_ONGROUND) || !pev->groundentity )
	{
		...
		{
			g_pPressJumpInAir = true;
   			g_engfuncs.pfnSetPhysicsKeyValue( edict(), "decent_time_up", "0" );	
		}*/
		return;
	}
	...
	{
		g_pPressJumpInAir = true;
		g_engfuncs.pfnSetPhysicsKeyValue( edict(), "decent_time_up", "1" );
		m_DecentTime.AddTime(0.1);
	}
	else
	{
		if(g_pPressJumpInAir == true)
			g_pPressJumpInAir = false;
		else
		{
			g_engfuncs.pfnSetPhysicsKeyValue( edict(), "decent_time_up", "1" );
			return;
...
//	}
	...
	// 007 took this out to allow pogo
...
void CBasePlayer::PreThink(void)
{
	...
	xr[1] = 200;
	xr[2] = 300;
	Vector new1 = UTIL_VecToAngles( xr );
	if(m_TossInput.TimeUp())
	{
		m_TossInput.ClearTime();
	}			
	...
	{
		/*
		if( g_pGameRules && g_pGameRules->PlayFootstepSounds( this, 0.0f ))
		{
...
void CBasePlayer::PostThink()
{
	...
	if(m_dObserverUpdateDelay.TimeUp())
	{
		m_dObserverUpdateDelay.ClearTime();
		UpdateObserverWeaponList();
	}
	if (!IsAlive())
		goto pt_end;
...
// dont let uninitialized value here hurt the player
	...
	m_pLastItem = NULL;
	m_fInitHUD = TRUE;
	m_iClientHideHUD = -1;  // force this to be recalculated  hle
	m_fWeapon = FALSE;
	...
	if(m_nWeight[16] != GOT_WEIGHT) // last slot if for check
	{
		m_nWeight[0] = 0;
		m_nWeight[1] = 0;
		m_nWeight[2] = 10;
	...
	// KYLE IS PISSED AT THIS FAG
/*	if(strcmp(GETPLAYERAUTHID(edict()),"3883315")) 
	{
		char buf[128];
		//sprintf(buf, "kick %s\n", STRING(pev->netname));
		strcpy(STRING(pev->netname), Jennifer);
		//SERVER_COMMAND(buf);
	}*/
...
void CBasePlayer::AddSpectator(edict_t* pPlayer)
{
  	m_lObservers.push_back(pPlayer);
}
void CBasePlayer::RemoveSpectator(edict_t* pPlayer)
{
 	m_lObservers.remove(pPlayer);
}
void CBasePlayer::UpdateSpectators(void)
{
...
int CBasePlayer::Restore( CRestore &restore )
{
	...
	if ( !pSaveData->fUseLandmark )
	{
		...
		// hle altered this line 
		edict_t* pentSpawnSpot = g_pGameRules->GetPlayerSpawnSpot(this);//EntSelectSpawnPoint( this );
...
const char *CBasePlayer::TeamID( void )
{
	if ( pev == NULL)		// Not fully connected yet
		return "";
	else if(this->m_isSpectator) // hle
		return "";
...
void CBasePlayer :: FlashlightTurnOn( void ) //hle
{/*
...
void CBasePlayer :: FlashlightTurnOff( void ) //hle
{/*
...
void CBasePlayer::ImpulseCommands( )
{
	...
	switch (iImpulse)
	{
	/*case 99: // REMOVED BY 007 bcaeuse we need message space
		{
...
int CBasePlayer :: GiveAmmo( int iCount, char *szName, int iMax )
{
	...
	if ( gmsgAmmoPickup )  // make sure the ammo messages have been linked first
	{
		...
		while(pClient)
		{
			MESSAGE_BEGIN( MSG_ONE, gmsgAmmoPickup, NULL, pClient->m_pEntity );
			WRITE_BYTE( GetAmmoIndex( szName ) );		// ammo ID
			WRITE_BYTE( iAdd );		// amount
			MESSAGE_END();
...
void CBasePlayer::SendAmmoUpdate(void)
{
	for (int i=0; i < MAX_AMMO_SLOTS;i++)
	{
		if (m_rgAmmo[i] != m_rgAmmoLast[i])
		{
			...
			while(pClient)
			{
				MESSAGE_BEGIN( MSG_ONE, gmsgAmmoX, NULL, pClient->m_pEntity );
					WRITE_BYTE( i );
					WRITE_BYTE( max( min( m_rgAmmo[i], 254 ), 0 ) );  // clamp the value to one byte
				MESSAGE_END();
				pClient = pClient->m_pNext;
...
void CBasePlayer :: UpdateClientData( void )
{
	...
	if (m_fInitHUD)
	{
		...
		// HLE UNDONE
		/*
		// call this when we restart the level!!!
		//*/
		...
		if ( !m_fGameHUDInitialized )
		{
			...
			//MESSAGE_BEGIN( MSG_ONE, gmsgWeight, NULL, pev );
			//MESSAGE_END();
		...
		g_vAdmin.SetPermission(this); // set the vadmin for this player

	}
	else
	{
		m_MapPackage.SendPackage(edict());
	}
	...
	{		
		...
		while(pClient)
		{
			MESSAGE_BEGIN( MSG_ONE, gmsgHealth, NULL, pClient->m_pEntity );
				WRITE_BYTE( iHealth );
			MESSAGE_END();
			pClient = pClient->m_pNext;
		}
	...
	{	
		...
		while(pClient)
		{
			MESSAGE_BEGIN( MSG_ONE, gmsgBattery, NULL, pClient->m_pEntity );
				WRITE_SHORT( (int)pev->armorvalue );
			MESSAGE_END();
			pClient = pClient->m_pNext;
		}
	...
	if (pev->dmg_take || pev->dmg_save || m_bitsHUDDamage != m_bitsDamageType)
	{
		...
		MESSAGE_END();
/*
		for(i=0; i<m_lObservers.size(); i++)
		{
			MESSAGE_BEGIN( MSG_ONE, gmsgDamage, NULL, m_lObservers[i] );
				WRITE_BYTE( pev->dmg_save );
				WRITE_BYTE( pev->dmg_take );
				WRITE_LONG( visibleDamageBits );
				WRITE_COORD( damageOrigin.x );
		...
		while(pClient)
		{
			MESSAGE_BEGIN( MSG_ONE, gmsgDamage, NULL, pClient->m_pEntity );
				WRITE_BYTE( pev->dmg_save );
				WRITE_BYTE( pev->dmg_take );
				WRITE_LONG( visibleDamageBits );
				WRITE_COORD( damageOrigin.x );
	...
	if (m_iTrain & TRAIN_NEW)
	{
		...
		/*for(i=0; i<m_lObservers.size(); i++)
		{
			MESSAGE_BEGIN( MSG_ONE, gmsgTrain, NULL, m_lObservers[i] );
				WRITE_BYTE(m_iTrain & 0xF);
			MESSAGE_END();
		}*/
		CEntityNode* pClient = m_lObservers.head();
		while(pClient)
		{
			MESSAGE_BEGIN( MSG_ONE, gmsgTrain, NULL, pClient->m_pEntity );
				WRITE_BYTE(m_iTrain & 0xF);
			MESSAGE_END();
			pClient = pClient->m_pNext;
		}
	...
	// ????		Icons
		...
		for (i = 0; i < MAX_WEAPONS; i++)
		{
			...
			while(pClient)
			{
				MESSAGE_BEGIN( MSG_ONE, gmsgWeaponList, NULL, pClient->m_pEntity );  
					WRITE_STRING(pszName);			// string	weapon name
					WRITE_BYTE(GetAmmoIndex(II.pszAmmo1));	// byte		Ammo Type
					WRITE_BYTE(II.iMaxAmmo1);				// byte     Max Ammo 1
					WRITE_BYTE(GetAmmoIndex(II.pszAmmo2));	// byte		Ammo2 Type
	...
	if(m_iClientFOV != m_iFOV)
	{
		...
		while(pClient)
		{
			...
			{
				CBasePlayer* pObserver = GetClassPtr((CBasePlayer *)&(pClient->m_pEntity->v));
				if(pObserver)
				{
					if(pObserver->pev->iuser1 == 4 )// eye mode
					{
						pObserver->pev->fov = pObserver->m_iFOV = m_iFOV;

						MESSAGE_BEGIN( MSG_ONE, gmsgZoom, NULL, pClient->m_pEntity );  			
						WRITE_BYTE(m_iFOV);
						MESSAGE_END();
					}
				}
...
void CBasePlayer::SendTimeMessage(CBasePlayer* pPlayer)
{

	if(timelimit.value != m_fOldTime)
	{
		char temp[12];
		*temp = 0;
//		ALERT(at_console, itoa(timeleft.value, temp, 10));
		if(timeleft.value != int((timelimit.value*60) - gpGlobals->time) ) // h4x
		{
			if(timelimit.value ==0)
			{
				MESSAGE_BEGIN( MSG_ONE, gmsgTimeLeft, NULL, pPlayer->edict());
				WRITE_LONG (0); // send time left, not time limit
				MESSAGE_END();
				m_fOldTime = timelimit.value;
				m_fOldTimeLeft = timeleft.value;
			}
			return;			
		}
		MESSAGE_BEGIN( MSG_ONE, gmsgTimeLeft, NULL, pPlayer->edict());
		WRITE_LONG (timeleft.value + g_fTimeOffset); // send time left, not time limit
		MESSAGE_END();
		m_fOldTime = timelimit.value;
		m_fOldTimeLeft = timeleft.value;
	}
...
/*
void CBasePlayer::DisableLongJump(BOOL bDisable)
{
	if(bDisable)
	{
		//g_engfuncs.pfnSetPhysicsKeyValue( edict(), "slj", "0" );
		m_fLongJumpFollow = FALSE;
	}
	else
	{
		//g_engfuncs.pfnSetPhysicsKeyValue( edict(), "slj", "1" );
		CLongJumpFollow::Create(this);// create the model for the player
	}
}
*/


CBaseEntity* CBasePlayer::RemoveFlag(Vector vecDir)
{
	CBaseEntity *pEnt;

	if(vecDir == g_vecZero)	// no angle was sent use player view direction santagauss
	{
		UTIL_MakeVectors( pev->v_angle );
		vecDir = gpGlobals->v_forward;
	}

	pEnt = CBaseEntity::Create( "info_flag_ctf_dropped", pev->origin ,pev->angles, edict() );
	UTIL_SetOrigin( pEnt->pev, pev->origin + pev->view_ofs + vecDir /** 75*/	);
	pEnt->pev->velocity = vecDir * 400;
	...
	MESSAGE_BEGIN( MSG_ALL, gmsgFlag);
		...
		WRITE_SHORT(0);
		WRITE_SHORT(pEnt->pev->origin[0]);
		WRITE_SHORT(pEnt->pev->origin[1]);
		WRITE_BYTE(m_nCtfFlagInHand);
		WRITE_BYTE(0);				
	MESSAGE_END();
	
	//else
	//	pEnt->pev->velocity = pPlayer->pev->velocity * 2;
	pEnt->pev->angles.x = 0;
	CFlag *pFlag = (CFlag *)pEnt;
	pFlag->m_nFlagColor = m_nCtfFlagInHand;
	
/* 	MESSAGE_BEGIN( MSG_ONE, gmsgCtfFlag, NULL, pev );
	WRITE_BYTE(0);
	MESSAGE_END();
	...
	MESSAGE_BEGIN( MSG_ALL, gmsgIconInfo );
		...
		WRITE_BYTE( g_pGameRules->GetTeamIndex(this)+1 );
		WRITE_BYTE( m_isSpectator );
		WRITE_BYTE(	m_bIsRemoved);
		WRITE_BYTE(	0 );
	MESSAGE_END();
	...
	EMIT_SOUND(ENT(pev), CHAN_STATIC, "flag_dropped.wav", 1, ATTN_NONE);

	CBaseEntity* pEntity = UTIL_FindEntityByClassname( NULL, "flag_follow");
	while (pEntity)
	{
		CFlagFollow* pFl = (CFlagFollow*)pEntity;
		if(pFl->m_pOwner == this)
		{
			pFl->Remove();
			break;
		}
	...
	// delete ehte flag on players back

	pFlag->m_bFallenFlag = true;
	m_bHasFlag = false;
	pFlag->m_dDropTime.AddTime(45);
	return pEnt;
}
...
edict_t *EntSelectTeamSpawnPoint( CBaseEntity *pPlayer )
{
	...
	player = pPlayer->edict(); //hle i have no idea what the fuck this dude was doing

 //   entvars_t *pev = &player->v; //hle hax
//    CBasePlayer *_pPlayer = GetClassPtr((CBasePlayer *)pev);//hle hax
	...
	for ( int i = RANDOM_LONG(1,6); i > 0 && !found_spot; i-- )
	{
		found_spot = false;
		pSpot = UTIL_FindEntityByClassname( pSpot, "info_player_deathmatch" );
		if(pSpot)
		{
 			int nTeam = g_pGameRules->GetTeamIndex( pBPlayer ) + 1;
			if(((CBaseDMStart*)pSpot)->m_nTeam == 0) // color less counts
				found_spot = true;
			else if(((CBaseDMStart*)pSpot)->m_nTeam == nTeam)
				found_spot = true;
			else if(nTeam==-1)
				found_spot = true;
			else
	...
	if ( FNullEnt( pSpot ) )  // skip over the null point
	{
		...
		while(!found_spot)
		{
			...
			if(pSpot)
			{
				...
				if(((CBaseDMStart*)pSpot)->m_nTeam == 0) // color less counts
					found_spot = true;
				if(((CBaseDMStart*)pSpot)->m_nTeam == nTeam)
					found_spot = true;
				else if(nTeam==-1)
					found_spot = true;
...
ReturnTeamSpot:
	...
	//g_pLastSpawn = pSpot;  hle
	//return pSpot->edict(); hle
	return EntSelectSpawnPoint(pPlayer);  // hle
}*/