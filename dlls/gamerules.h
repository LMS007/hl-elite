...
public:
	...
	virtual int CheckValidTeam(char* pCurrentTeam, char* pNewTeam)=0;
	virtual void SendScoreInfo(CBasePlayer* pSender, CBasePlayer* pReciever=NULL)=0;
	virtual BOOL MatchMode()=0;
	virtual void ResumePlayer(CBasePlayer* pPlayer) =0;
	virtual void ResetPlayerHud(CBasePlayer* pPlayer) =0;
	virtual void SetSpectatorHud(CBasePlayer* pPlayer, CBasePlayer* pTarget)=0;
	virtual void RemovePlayer(CBasePlayer* pPlayer) =0;
	virtual BOOL MatchInProgress(void) = 0;
	virtual void MatchStart( int nType ) = 0;
	virtual BOOL CanStartObserver(CBasePlayer *pPlayer) = 0;
	virtual BOOL CanStopObserver(CBasePlayer *pPlayer) = 0;
	virtual BOOL CanPlayerSpawn(CBasePlayer *pPlayer) = 0; // hle
	virtual void StartObserver(CBasePlayer *pPlayer)=0;
	virtual void CapturedFlag(CBasePlayer *pPlayer) = 0;
	virtual BOOL CanStartAutoObserver(CBasePlayer* pPlayer) = 0;
	virtual BOOL RestrictObserver( void ) = 0;
	virtual void AddClock(float)=0;
...
// Functions to verify the single/multiplayer status of a game
	...
	virtual const char *GetGameDescription( void ) { return "HLE FFA"; }  // this is the game name that gets seen in the server browser
...
// Client spawn/respawn control
	...
	virtual edict_t *GetRandomSpawn( CBasePlayer *pPlayer )=0; // hle, this function is designed for each game mode to pull a correct spawn, via team or non team
	virtual void InitSpawns()=0;
...
// Teamplay stuff
	...
	virtual int GetTeamIndex( CBasePlayer *pPlayer ) { return -1; }
	virtual int GetTeamIndex2( const char *pTeamName ) { return -1; }
	virtual const char *GetIndexedTeamName( int teamIndex ) { return ""; }
	virtual BOOL IsValidTeam( const char *pTeamName ) { return TRUE; }
	virtual int ChangePlayerTeam( CBasePlayer *pPlayer, const char *pTeamName, BOOL bKill, BOOL bGib ) {return 0;}
...
public:
	...
	virtual BOOL CanPlayerSpawn( CBasePlayer* pPlayer );
