...
void CCrossbowBolt::BoltTouch( CBaseEntity *pOther )
{
	...
	if (pOther->pev->takedamage)
	{
		...
		if ( pOther->IsPlayer() )
		{
			pOther->TraceAttack(pevOwner, CROSSBOW_EXPLODE_DMG, pev->velocity.Normalize(), &tr, DMG_NEVERGIB ); 
		}
		else
		{
...
//	p->iWeight = m_pPlayer->m_nWeight[WEAPON_CROSSBOW];
...
void CCrossbow::FireSniperBolt()
{
	...
	{
		ClearMultiDamage( );
		CBaseEntity::Instance(tr.pHit)->TraceAttack(m_pPlayer->pev, CROSSBOW_SNIPE_DMG, vecDir, &tr, DMG_BULLET | DMG_NEVERGIB ); //set it too 100 here to make sure santagauss
		ApplyMultiDamage( pev, m_pPlayer->pev );
	}
// new santagauss sky event
	...
	{
		PLAYBACK_EVENT_FULL( 0, m_pPlayer->edict(), m_usCrossbow2, 0.0, (float *)&tr.vecEndPos, (float *)&g_vecZero, 0.0, 0.0, 0, 0, 1, 0 );
	}
	else
	{
		PLAYBACK_EVENT_FULL( 0, ENT(tr.pHit), m_usCrossbow2, 0.0, (float *)&tr.vecEndPos, (float *)&g_vecZero, 0.0, 0.0, 0, 0, 1, 1 );
