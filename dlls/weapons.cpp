...
void DecalGunshot( TraceResult *pTrace, int iBulletType )
{
	...
	if ( VARS(pTrace->pHit)->solid == SOLID_BSP || VARS(pTrace->pHit)->movetype == MOVETYPE_PUSHSTEP )
	{
		...
		// start op4 weapons - santagauss maybe take out?
...
void W_Precache(void)
{
	...
	UTIL_PrecacheOther( "item_healthkit" );	// new fix cheat crash - santagauss
...
{
	if(m_fTime!=0) // hack to get around first materilize
	{
	...
	SetTouch (&CBasePlayerItem::DefaultTouch);
	SetThink ( &CBasePlayerItem::DefaultThink );

}

void CBasePlayerItem::DefaultThink( void )
{
	if(pev->spawnflags & SF_NORESPAWN)
	{
...
/*
CBasePlayerItem* CBasePlayerItem::TossItem()
{
	CBaseEntity *pEnt;
	...
	if(m_pPlayer)
	{
		...
		pEnt = CBaseEntity::Create( (char*)STRING(pev->classname), m_pPlayer->pev->origin ,m_pPlayer->pev->angles, m_pPlayer->edict() );
		UTIL_SetOrigin( pEnt->pev, m_pPlayer->pev->origin + m_pPlayer->pev->view_ofs + gpGlobals->v_forward * 64);
	...
	return (CBasePlayerItem*)pEnt;
}*/
CBaseEntity* CBasePlayerWeapon::TossWeapon()
{
	CBasePlayerWeapon* pWpn = NULL;//(CBasePlayerWeapon*)CBasePlayerItem::TossItem();
	...
	if(m_pPlayer)
	{
		...
		pEnt->pev->spawnflags |= SF_NORESPAWN;
		pWpn = (CBasePlayerWeapon*)pEnt;

		ammoleft = m_pPlayer->m_rgAmmo[ m_pPlayer->GetAmmoIndex(pszAmmo1()) ];
		bool weapons_left = false;

		if(FStrEq((char*)STRING(pev->classname), "weapon_snark"))
		{
			if(m_pPlayer->m_rgAmmo[ m_pPlayer->GetAmmoIndex(pszAmmo1())] > 1)
				weapons_left = true;
		}
		else if(FStrEq((char*)STRING(pev->classname), "weapon_handgrenade"))
		{
			if(m_pPlayer->m_rgAmmo[ m_pPlayer->GetAmmoIndex(pszAmmo1())] > 1)
				weapons_left = true;
		}
		else if(FStrEq((char*)STRING(pev->classname), "weapon_tripmine"))
		{
			if(m_pPlayer->m_rgAmmo[ m_pPlayer->GetAmmoIndex(pszAmmo1())] > 1)
				weapons_left = true;
		}
		else if(FStrEq((char*)STRING(pev->classname), "weapon_satchel"))
		{
			if(m_pPlayer->m_rgAmmo[ m_pPlayer->GetAmmoIndex(pszAmmo1())] > 1)
				weapons_left = true;
		}
		else if(FStrEq((char*)STRING(pev->classname), "weapon_rpg"))
		{
		...
		*/
		...
		else 
		{
			if(m_pPlayer)
			{
				...
				if(ammoleft < pWpn->m_iDefaultAmmo_stored)
				{
					m_pPlayer->m_rgAmmo[ m_pPlayer->GetAmmoIndex(pszAmmo1())] = 0;
					pWpn->SetDefaultAmmo(ammoleft);
				}
				else
				{
					m_pPlayer->m_rgAmmo[ m_pPlayer->GetAmmoIndex(pszAmmo1())] -= m_iDefaultAmmo_stored;
					pWpn->SetDefaultAmmo(m_iDefaultAmmo_stored);
				}
			}
		}	
		if(!weapons_left)
...
int CBasePlayerWeapon::UpdateClientData( CBasePlayer *pPlayer )
{
	...
	if ( bSend )
	{
		...
		while(pClient)
		{
			MESSAGE_BEGIN( MSG_ONE, gmsgCurWeapon, NULL, pClient->m_pEntity );
				WRITE_BYTE( state );
				WRITE_BYTE( m_iId );
				WRITE_BYTE( m_iClip );
			MESSAGE_END();
...
#ifndef CLIENT_DLL
		if(m_pPlayer->m_pActiveItem && !FStrEq(STRING(m_pPlayer->m_pActiveItem->pev->classname), "weapon_egon"))
		{
			CEntityNode* pClient = m_pPlayer->m_lObservers.head();
			while(pClient)
			{
				MESSAGE_BEGIN( MSG_ONE, SVC_WEAPONANIM, NULL, pClient->m_pEntity );
...
{
	// new fixes auto switch when a player still has secondary ammo - santagauss
	if ( pszAmmo2() && (m_pPlayer->m_rgAmmo[m_iSecondaryAmmoType] != 0) )
	{
		return TRUE;
	}
	// end new
...
BOOL CBasePlayerWeapon :: DefaultDeploy( char *szViewModel, char *szWeaponModel, int iAnim, char *szAnimExt, int skiplocal /* = 0 */, int body )
{
	...
	m_flLastPrimaryFireTime = 0.0;
	m_flLastSecondaryFireTime = 0.0;
	return TRUE;
}
...
//=========================================================================
//  An accurate way of calcualting the next attack time.
//=========================================================================
/*void CBasePlayerWeapon::SetNextPrimaryAttackDelay(float delay) {
	
	if(m_flLastPrimaryFireTime == 0 || m_flNextPrimaryAttack == -1) 
	{
		// At this point, we are assuming that the client has stopped firing
		// and we are going to reset our book keeping variables.
		m_flLastPrimaryFireTime = gpGlobals->time;
		m_flPrevPrimaryAttack = delay;
	}
	// calculate the time between this shot and the previous
	float flTimeBetweenFires = gpGlobals->time - m_flLastPrimaryFireTime;
	float flCreep = 0.0f;
...
void CBasePlayerWeapon::SetNextPrimaryAttackDelay(float delay) {
	// stop time for primary seems to be -1
	SetNextAttackDelay(delay, &m_flLastPrimaryFireTime, &m_flNextPrimaryAttack, &m_flPrevPrimaryAttack, -1.0);
}
void CBasePlayerWeapon::SetNextSecondaryAttackDelay(float delay) {
	// stop time for primary seems to be -0.001
	SetNextAttackDelay(delay, &m_flLastSecondaryFireTime, &m_flNextSecondaryAttack, &m_flPrevSecondaryAttack, -0.001);
}
void CBasePlayerWeapon::SetNextAttackDelay( float delay, float* lastFireTime, float* nextAttack, float* prevAttack, float stopTime) {
	
	float l = *lastFireTime;
	float na = *nextAttack;
	if(*lastFireTime == 0 || (gpGlobals->time - *lastFireTime) > delay + .05)
		  // -1 for primary, -0.001 for secondary.
	//if(*lastFireTime == 0 || *nextAttack == stopTime)  // -1 for primary, -0.001 for secondary.
	{
		// At this point, we are assuming that the client has stopped firing
		// and we are going to reset our book keeping variables.
		*lastFireTime = gpGlobals->time;
		*prevAttack = delay;
	}
	// calculate the time between this shot and the previous
	float flTimeBetweenFires = gpGlobals->time - *lastFireTime;
	float flCreep = 0.0f;
	if(flTimeBetweenFires > 0)
		flCreep = flTimeBetweenFires - *prevAttack; // postive or negative
	...
	*lastFireTime = gpGlobals->time;		
	...
	// we need to remember what the m_flNextPrimaryAttack time is set to for each shot, 
	// store it as m_flPrevPrimaryAttack.
	*prevAttack = flNextAttack;
	*nextAttack = flNextAttack;

	char buf[1500];
	*buf = NULL;
	if(&m_flNextPrimaryAttack == nextAttack)
		sprintf(buf, "P betwen %.4f lasttime%.4f nxt %.4f creep %.4f", flTimeBetweenFires, l, na, flCreep);
		//sprintf(buf, "PRIME FIRE: delay: %.4f NEW nextAttack: %.4f OLD nextAttack: %.4f flCreep: %.4f flTimeBetweenFires: %.4f Last Fire Time: %.4f\n", delay, nextAttack, oldNext, flCreep, flTimeBetweenFires, l);
	else
		sprintf(buf, "S betwen %.4f lasttime%.4f nxt %.4f creep %.4f", flTimeBetweenFires, l, na, flCreep);
	ClientPrint( m_pPlayer->pev, HUD_PRINTTALK, buf);
}
...
void CBasePlayerAmmo::Spawn( void )
{
	...
	SetThink( &CBasePlayerAmmo::DefaultThink );
	pev->nextthink = gpGlobals->time + .1;
}
...
void CBasePlayerAmmo :: DefaultThink(  )
{
	if(pev->spawnflags & SF_NORESPAWN)
	{
		if(m_fTime < gpGlobals->time )
...
void CBasePlayerAmmo :: DefaultTouch( CBaseEntity *pOther )
{
	...
	if (AddAmmo( pOther ))
	{
		...
		else if ( g_pGameRules->AmmoShouldRespawn( this ) == GR_AMMO_RESPAWN_YES )
		{
			Respawn();
		}
		else
...
int CBasePlayerWeapon::ExtractAmmo( CBasePlayerWeapon *pWeapon )
{
	...
	if ( pszAmmo1() != NULL )
	{
		...
		m_iDefaultAmmo_stored = m_iDefaultAmmo;
		m_iDefaultAmmo = 0; //hle
	}
...
void CBasePlayerWeapon::PrintState( BOOL miss )
{
