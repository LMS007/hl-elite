...
void CTripmineGrenade :: MakeBeam( void )
{
	 TraceResult tr;
	    ...
	    m_pBeam = CBeam::BeamCreate( "sprites/smoke.spr", 15 );
	    m_pBeam->PointEntInit( vecTmpEnd, entindex() );
		m_pBeam->SetScrollRate( 255 );
 		m_pBeam->SetBrightness( 255 );
		m_pBeam->SetColor( 100, 180, 255 );
/*
#ifdef CLIENT_DLL
		if(hle_teamplay == 2)//CTF
#else
		if(gamemode.value = 4)
#endif
		{
			...
			CBasePlayer* pWnp = GetClassPtr((CBasePlayer*)pOwner);
			//m_hOwner->

			if(pWnp->m_pPlayer->m_cCtfTeam == 'r')
				m_pBeam->SetColor( 255, 50, 50 );
			else
				m_pBeam->SetColor( 50, 150, 255 );
		}		
//#else
		if(gamemode.value == CTF)
		{
			if(m_pPlayer->m_cCtfTeam = RED)
				m_pBeam->SetColor( 255, 50, 50 );
			else
				m_pBeam->SetColor( 50, 150, 255 );
		}	
...
//	p->iWeight = m_pPlayer->m_nWeight[WEAPON_TRIPMINE];
...
void CTripmine::PrimaryAttack( void )
{
	...
	// TOTAL TEST find out how many mines i have out. do it this way to make sure - santagauss
#ifndef CLIENT_DLL
	int iHowManyOut = 0;
	edict_t *pPlayer = m_pPlayer->edict( );
	CBaseEntity *pEntOut = NULL;

	while ((pEntOut = UTIL_FindEntityInSphere( pEntOut, Vector(0,0,0), 6144 )) != NULL)
	{
		if (FClassnameIs( pEntOut->pev, "monster_tripmine"))
		{
			CTripmineGrenade *pTripOut = (CTripmineGrenade*)pEntOut;

			if (pTripOut->m_pRealOwner == pPlayer)
			{
				iHowManyOut++;
			}
		}
	...
	if (tr.flFraction < 1.0)
	{
		...
		if ( pEntity && !(pEntity->pev->flags & FL_CONVEYOR) && pEntity->IsBSPModel() )	// new santagauss is bsp & sky
		{
#ifndef CLIENT_DLL
...
//		else	{ ALERT( at_console, "no deploy\n" ); }
	}
	//m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 0.3;
	SetNextPrimaryAttackDelay(0.3);
