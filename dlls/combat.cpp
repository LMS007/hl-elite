...
// i changed this to be more realistic around water, and fix some bugs - santagauss
// also to override the hit group for gauss
	
void RadiusDamage( Vector vecSrc, entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, float flRadius, int iClassIgnore, int bitsDamageType, int forceBodyGroup)
{
	...
	while ((pEntity = UTIL_FindEntityInSphere( pEntity, vecSrc, flRadius )) != NULL)
	{
		if ( pEntity->pev->takedamage != DAMAGE_NO )
		{
			...
			{// the explosion can 'see' this entity, so hurt them!
				...
				if(bDiffMedium)
					flAdjustedDamage *= 0.5; // decrease for water difference
				...
				if(forceBodyGroup)	// new - santagauss allows you to override the hit group mostly for getting rid of head shot multiplier.
					tr.iHitgroup = forceBodyGroup;
				if (tr.flFraction != 1.0)
				{
					ClearMultiDamage( );
...
================
*/
Vector CBaseEntity::FireBulletsPlayer ( ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker, int shared_rand, float penetration)	// m249 this whole function
{
	static int tracerCount;
	TraceResult tr;
	Vector vecRight = gpGlobals->v_right;
	...
	for ( ULONG iShot = 1; iShot <= cShots; iShot++ )
	{
		...
		if (tr.flFraction != 1.0)
		{
			...
			if(penetration)
			{	// this lets bullets penetrate walls it may need to be modified in the future for other bullets. - santagauss

				//int maxPointChecks = 12/penetration;
				int maxPointChecks = 4;
				int pointChecks = 0;
				Vector vecWGpoint = tr.vecEndPos;
				
				while(pointChecks <= maxPointChecks)
				{	// check for a open spot - santagauss
					pointChecks++;

					vecWGpoint = vecWGpoint + vecDir * 12;

					if(UTIL_PointContents(vecWGpoint) != CONTENT_SOLID)
					{
						

						vecEnd = vecWGpoint + vecDir * flDistance;
						UTIL_TraceLine(vecWGpoint, vecEnd, dont_ignore_monsters, ENT(pev)/*pentIgnore*/, &tr);
