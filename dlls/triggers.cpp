...
public:
	...
	void EXPORT PreTeleportTouch(CBaseEntity *);
	void Spawn( void );
};
...
void CTriggerTeleport::KeyValue(KeyValueData* pkvd)
{
	...
	if (FStrEq(pkvd->szKeyName, "team_teleport"))
	{
...
void CTriggerTeleport::PreTeleportTouch(CBaseEntity *pOther)
{

	if ( !pOther->IsPlayer() )
		return;

	// this it to set up team teleports for Ctf
	CBasePlayer *pPlayer = (CBasePlayer *)pOther;
	if(g_teamplay) 
	{
		if(nTeam != NULL) // team teleport
		{
			if(g_pGameRules->GetTeamIndex( pPlayer ) + 1 != nTeam)
				return;
		}
	}               
