...
LINK_ENTITY_TO_CLASS( weapon_eagle, CPython ); // new - santagauss
...
//	p->iWeight = m_pPlayer->m_nWeight[WEAPON_PYTHON];
...
void CPython::PrimaryAttack()
{
	...
	if (m_iClip <= 0)
	{
		...
		else
		{
			...
			m_flNextPrimaryAttack = 0.15;//hle 0.15
		}*/
	...
	Vector vecDir;
	vecDir = m_pPlayer->FireBulletsPlayer( 1, vecSrc, vecAiming, VECTOR_CONE_1DEGREES, 8192, BULLET_PLAYER_357, 0, PYTHON_DMG, m_pPlayer->pev, m_pPlayer->random_seed, 0);
	...
	//m_flNextPrimaryAttack = 0.55;//hle 0.75
	SetNextPrimaryAttackDelay(0.55);
...
void CPython::Reload( void )
{
	...
	if (DefaultReload( 6, PYTHON_RELOAD, 3.0, bUseScope )) //1 = 2 hle
	{
		m_flSoundDelay = 1.5;
	}
}
