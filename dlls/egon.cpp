...
#define EGON_NO_FIRE -1
#define EGON_PRIMARY_FIRE		0
#define EGON_SECONDARY_FIRE		1
...
#ifndef CLIENT_DLL
//	if(m_pPlayer->m_fLongJump)
//		m_pPlayer->DisableLongJump(TRUE);
#endif
...
void CEgon::Holster( int skiplocal /* = 0 */ )
{
	...
	while(pClient)
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgFixEgon, NULL, pClient->m_pEntity );
		MESSAGE_END();
		pClient = pClient->m_pNext;
	}
//	if(m_pPlayer->m_fLongJump)
//		m_pPlayer->DisableLongJump(FALSE);
#endif
}
...
//	p->iWeight = m_pPlayer->m_nWeight[WEAPON_EGON];
...
/*
UpdateBeam(gpGlobals->v_forward)
{

}*/
void CEgon::Attack( bool bDamageType ) // 0 hurt 1 for life
{


		//bDamageType = 0; // cant do team hev in ohter game mode
	...
	switch( m_fireState )
	{
		case FIRE_OFF:
		{
			if ( !HasAmmo() )
			{
				...
				SetNextSecondaryAttackDelay(0.25);
				PlayEmptySound( );
				return;
			}
			...
			else // secondary
			{
				...
				//PLAYBACK_EVENT_FULL( flags, m_pPlayer->edict(), m_usEgonFire2, 0.0, (float *)&g_vecZero, (float *)&g_vecZero, 0.0, 0.0, m_fireState, m_fireMode, 1, 0 );
			}
		...
		case FIRE_CHARGE:
		{
			Fire( vecSrc, vecAiming, bDamageType);
			m_pPlayer->m_iWeaponVolume = EGON_PRIMARY_VOLUME;
			...
			if ( pev->fuser1 <= UTIL_WeaponTimeBase() )
			{
				if(!bDamageType) // primary
				{
					//ALERT(at_console, "event 3 ");
					PLAYBACK_EVENT_FULL( flags, m_pPlayer->edict(), m_usEgonFire, 0, (float *)&g_vecZero, (float *)&g_vecZero, 0.0, 0.0, m_fireState, m_fireMode, 0, 0 ); // 0
				}
				else
				{
					ALERT(at_console, "event 4 ");
					PLAYBACK_EVENT_FULL( flags, m_pPlayer->edict(), m_usEgonFire2, 0, (float *)&g_vecZero, (float *)&g_vecZero, 0.0, 0.0, m_fireState, m_fireMode, 0, 0 ); // 0
				}
				pev->fuser1 = 1000;
			}
			...
			if ( !HasAmmo() )
			{
				...
				SetNextPrimaryAttackDelay(1.0);
				SetNextSecondaryAttackDelay(1.0);
			}
...
{
	if(m_nFireType == EGON_SECONDARY_FIRE) // hax to prevent primary/secondary beam color change
	{
		EndAttack();
	}
	else
	{
		m_nFireType = EGON_PRIMARY_FIRE;
		m_fireMode = FIRE_WIDE;
		Attack(EGON_PRIMARY_FIRE);
	}
...
#endif
*/
	if(m_nFireType == EGON_PRIMARY_FIRE) // hax to prevent primary/secondary beam color change
	{
		EndAttack();
	}/*
	else
	{
		m_nFireType = EGON_SECONDARY_FIRE;
		m_fireMode = FIRE_WIDE;
		Attack(EGON_SECONDARY_FIRE);
	}*/

}
void CEgon::Fire( const Vector &vecOrigSrc, const Vector &vecDir, bool bDamageType )
{
	...
	case FIRE_NARROW:
		...
		if ( pev->dmgtime < gpGlobals->time )
		{
			...
			ClearMultiDamage();
			if(bDamageType == EGON_SECONDARY_FIRE)
			{
				/*if(pEntity->IsPlayer())
				{
					CBasePlayer* pPlayer = (CBasePlayer*)pEntity;
					if(pPlayer->m_szTeamName == m_pPlayer->m_cCtfTeam)
					{
			...
			else
			{
				if (pEntity->pev->takedamage)
					pEntity->TraceAttack( m_pPlayer->pev, EGONNARROW_DMG, vecDir, &tr, DMG_ENERGYBEAM );
	...
	case FIRE_WIDE:
		...
		if ( pev->dmgtime < gpGlobals->time )
		{
			...
			if(bDamageType == EGON_SECONDARY_FIRE)
			{
				...
				{
					CBasePlayer* pPlayer = (CBasePlayer*)pEntity;
/*					if(pPlayer->m_cCtfTeam == m_pPlayer->m_cCtfTeam)
					{
						if(pPlayer->pev->armorvalue < 100)
							pPlayer->pev->armorvalue+=1;
					}*/
				}
				else if(pEntity->pev == m_pPlayer->pev)
				{
					int i = 5;
				}
			}
			else
			{
				if (pEntity->pev->takedamage)
				{
					pEntity->TraceAttack( m_pPlayer->pev, EGONWIDE_DMG, vecDir, &tr, DMG_ENERGYBEAM | DMG_ALWAYSGIB);
				}
				ApplyMultiDamage(m_pPlayer->pev, m_pPlayer->pev);
			}
			...
			if ( g_pGameRules->IsMultiplayer() )
			{
				...
				if(bDamageType == EGON_PRIMARY_FIRE) // if primary fire only
					::RadiusDamage( tr.vecEndPos, pev, m_pPlayer->pev, EGONWIDE_DMG/4, 128, CLASS_NONE, DMG_ENERGYBEAM | DMG_BLAST | DMG_ALWAYSGIB );
			}
			...
			if ( g_pGameRules->IsMultiplayer() )
			{
				...
				{
					UseAmmo( 1 );
					m_flAmmoUseTime = gpGlobals->time + 0.1; // 0.2
				}
...
void CEgon::UpdateEffect( const Vector &startPoint, const Vector &endPoint, float timeBlend )
{
	...
	{
		if(m_nFireType == EGON_SECONDARY_FIRE)
			m_pBeam->SetColor( 30 + (25*timeBlend), 64 + 80*fabs(sin(gpGlobals->time*10)), 30 + (30*timeBlend) );
		else
	...
	{
		if(m_nFireType == EGON_SECONDARY_FIRE)
			m_pBeam->SetColor( 60 + (25*timeBlend), 64 + 80*fabs(sin(gpGlobals->time*10)), 120 + (30*timeBlend));
		else
...
void CEgon::CreateEffect( void )
{
	...
	m_pBeam->pev->flags |= FL_SKIPLOCALHOST; // hle
	...
	m_pSprite->pev->flags |= FL_SKIPLOCALHOST; // hle
...
void CEgon::EndAttack( void )
{
	...
	SetNextPrimaryAttackDelay(0.4);
	SetNextSecondaryAttackDelay(0.4);
