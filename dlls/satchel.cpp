...
class CSatchelCharge : public CGrenade
{
	...
	int TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType ); // hle
...
{
	if(pevAttacker != NULL)	// give damage to detonator. - santagauss
...
//	p->iWeight = m_pPlayer->m_nWeight[WEAPON_SATCHEL];
...
void CSatchel::PrimaryAttack()
{
	...
	case 1:
		...
		while ((pSatchel = UTIL_FindEntityInSphere( pSatchel, m_pPlayer->pev->origin, 11776 )) != NULL)
		{
			if (FClassnameIs( pSatchel->pev, "monster_satchel"))
			{
				CSatchelCharge *pSatchelOut = (CSatchelCharge*)pSatchel;
				if (pSatchelOut->pev->owner == pPlayer)
				{
					pSatchelOut->Use( m_pPlayer, m_pPlayer, USE_ON, 0 );
					m_chargeReady = 2;
				}
			}
		}
...
void CSatchel::Throw( void )
{
	...
	// TOTAL TEST find out how many satchels i have out. do it this way to make sure - santagauss
#ifndef CLIENT_DLL
	int iHowManyOut = 0;
	edict_t *pPlayer = m_pPlayer->edict( );
	...
	{
		if (FClassnameIs( pEntOut->pev, "monster_satchel"))
		{
			CSatchelCharge *pSatchelOut = (CSatchelCharge*)pEntOut;
			if (pSatchelOut->pev->owner == pPlayer)
			{
				iHowManyOut++;
			}
	...
	if(iHowManyOut)
		m_chargeReady = 1;	// if we drop, then try to pick up again. 

	if(iHowManyOut >= SATCHEL_MAX_CARRY)
		return;
#endif
	// END TOTAL TEST
