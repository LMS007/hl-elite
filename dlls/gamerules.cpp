...
extern CDelayTime g_SlowFrameThink;
...
// PLAYER WEAPONS
	...
	// Crossbow
	gSkillData.plrDmgCrossbow = GetSkillCvar( "sk_plr_xbow_bolt_client");
...
{	
	...
	g_SlowFrameThink.ClearTime();
	g_SlowFrameThink.AddTime(1);

	g_PracticeMode = 0;
	
//	ifstream config;
//	ofstream newconfig;
//	ifstream default_config;
	////////////////////////////////////
	// set game parameters from text file
	if(FStrEq(gamemode.string, "practice"))
	{
	}
	else if(g_nMapMode == FORCE_CTF)
	{
		if(FStrEq(gamemode.string, "ctp") || FStrEq(gamemode.string, "teamplay") || FStrEq(gamemode.string, "ffa"))
			CVAR_SET_STRING("mp_gamemode", "ctf");
			
	}
			
	else if(g_nMapMode == FORCE_DUEL)
	{
		if(  !FStrEq(gamemode.string, "action") 
			&& !FStrEq(gamemode.string, "lms") 
			&& !FStrEq(gamemode.string, "lts") 
			&& !FStrEq(gamemode.string, "teamaction")
			&& !FStrEq(gamemode.string, "teamduel")) // action, lms mode is allowed otherwise its duel mode
	...
	else if(g_nMapMode == FORCE_TOURNAMENT)
	{
		if(FStrEq(gamemode.string, "ctf"))
			CVAR_SET_STRING("mp_gamemode", "tournament");
			//strcpy(gamemode.string,"tournament"); // ctf is not allowed 
	}
	else if( g_nMapMode == FORCE_ALL)
	{
		if(FStrEq(gamemode.string, "ctf")) // ctf is now allowed 
		{
			
			if(tournament.value==0)
			{
				SERVER_COMMAND( "exec game.cfg\n" ); // try to replace with game.cfg
				SERVER_EXECUTE( );
			}
			if(FStrEq(gamemode.string, "ctf")) 
			{
				CVAR_SET_STRING("mp_gamemode", "ffa");
//				strcpy(gamemode.string,"ffa");  // if its still ctf then force to ffa
			}
		}
	}
	...
	{
		g_PracticeMode = 1;
		return new CGameModePractice( ); 
	}
	if(FStrEq(gamemode.string, "ffa"))
	{
		return new CHalfLifeMultiplay( ); 
	}
	else if(FStrEq(gamemode.string, "action"))
	{
		return new CGameModeAction( );
	}
	else if(FStrEq(gamemode.string, "duel"))
	{
		return new CGameModeDuel( );
	}
	else if(FStrEq(gamemode.string, "teamplay"))
	{
		g_teamplay = 1;
		return new CHalfLifeTeamplay();
	}
	else if(FStrEq(gamemode.string, "ctf"))
	{
		g_teamplay = 1;
		return new CGameModeCTF( );
	}
	else if(FStrEq(gamemode.string, "lms"))
	{
		return new CGameModeLMS( );
	}
	else if(FStrEq(gamemode.string, "tournament"))
	{
		return new CGameModeTourney();
	}
	else if(FStrEq(gamemode.string, "ctp"))
	{
		g_teamplay = 1;
		return new CGameModeCTP();
	}
	else if(FStrEq(gamemode.string, "lts"))
	{
		g_teamplay = 1;
		return new CGameModeTLMS();
	}
	else if(FStrEq(gamemode.string, "teamaction"))
	{
		g_teamplay = 1;
		return new CHalfLifeTAction();
	}
	else if(FStrEq(gamemode.string, "teamduel"))
	{
		g_teamplay = 1;
		return new CGameModeTeamDuel();
	}
	else // default FFA
	{
		return new CHalfLifeMultiplay( );
