...
//extern char** GetMapList ( int* m_pNumberOfMaps );
//extern void SendMapList ( edict_t *client, char** szMapList, int m_nSize );
...
extern CVoteStructure *g_pVote; // hle
...
//    m_bMapListInIt = FALSE;
	...
	else
	{
		...
		if ( lservercfgfile && lservercfgfile[0] )
		{
			...
			debug_file.OpenFile();
			debug_file << "CHalfLifeMultiplay :: CHalfLifeMultiplay()\n";
...
// damages for weapons new - santagauss


		// Crowbar whack
	gSkillData.plrDmgCrowbar = CROWBAR_DMG;
	gSkillData.plrDmg9MM = GLOCK_DMG;
	gSkillData.plrDmg357 = PYTHON_DMG; // 40
	gSkillData.plrDmgMP5 = MP5_DMG;
	gSkillData.plrDmgM203Grenade = M203GRENADE_DMG;
	gSkillData.plrDmgBuckshot = BUCKSHOT_DMG;// fewer pellets in deathmatch
	gSkillData.plrDmgCrossbow = CROSSBOW_SNIPE_DMG;//20;
	gSkillData.plrDmgCrossbowExplode = CROSSBOW_EXPLODE_DMG;
	gSkillData.plrDmgRPG = RPG_DMG;
	gSkillData.plrDmgEgonWide = EGONWIDE_DMG; 
	gSkillData.plrDmgEgonNarrow = EGONNARROW_DMG; 
	gSkillData.plrDmgGauss = GAUSS_DMG;
	gSkillData.plrDmgHandGrenade = HANDGRENADE_DMG;
	gSkillData.plrDmgHandGrenadeHit = HANDGRENADE_HIT_DMG;
	gSkillData.plrDmgTripmine = TRIPMINE_DMG;
	gSkillData.plrDmgHornet = HORNET_DMG;
	gSkillData.plrDmgShock = SHOCK_BEAM_DMG;
	gSkillData.plrDmgM249 = BULLET_556_DMG;
	gSkillData.plrDmgSpore = SPORE_DMG;
	gSkillData.plrDmgSniper = BULLET_762_DMG;
	gSkillData.plrDmgSatchel = SATCHEL_DMG;
...
void CHalfLifeMultiplay::InitSpawns()
{
	m_pSpawnPoints.CreateSpawnList();
}
void CHalfLifeMultiplay :: Think ( void )
{
	...
	static int last_wins;

	int frags_remaining = 0;
	int time_remaining = 0;
	int wins_remaining = 0;
	...
	float flWinLimit = winlimit.value;
	...
	if ( flWinLimit ) // hle
	{
		...
		// check if any player is over the win limit
		for ( int i = 1; i <= gpGlobals->maxClients; i++ )
		{
			...
			if(pEntity)
			{
				...
				{
					remain = flWinLimit - pPlayer->m_iWins;
					if ( remain < bestwins )
					{
						bestwins = remain;
					}
		...
		wins_remaining = bestwins;
	}

	// Updates when frags change
	if ( wins_remaining != last_wins )
	{
	//	g_engfuncs.pfnCvar_DirectSet( &winsleft, UTIL_VarArgs( "%i", wins_remaining ) );
	}
...
BOOL CHalfLifeMultiplay::FShouldSwitchWeapon( CBasePlayer *pPlayer, CBasePlayerItem *pWeapon )
{
	...
	if(pPlayer->m_nWeight[pWeapon->m_iId] > pPlayer->m_nWeight[pPlayer->m_pActiveItem->m_iId])
	{
		return TRUE;
	}
...
BOOL CHalfLifeMultiplay :: GetNextBestWeapon( CBasePlayer *pPlayer, CBasePlayerItem *pCurrentWeapon )
{
	...
	for ( i = 0 ; i < MAX_ITEM_TYPES ; i++ )
	{
		...
		while ( pCheck )
		{
			if(pPlayer->m_nWeight[pCheck->m_iId] > -1 && pPlayer->m_nWeight[pCheck->m_iId] ==  pPlayer->m_nWeight[pCurrentWeapon->m_iId] && pCheck != pCurrentWeapon)
			...
			else if(pPlayer->m_nWeight[pCheck->m_iId] > iBestWeight && pCheck != pCurrentWeapon ) // hle
			//else if ( pCheck->iWeight() > iBestWeight && pCheck != pCurrentWeapon )// don't reselect the weapon we're trying to get rid of
			{
				...
				if ( pCheck->CanDeploy() )
				{
					...
					iBestWeight = pPlayer->m_nWeight[pCheck->m_iId];
					pBest = pCheck;
				}
			}
...
void CHalfLifeMultiplay :: InitHUD( CBasePlayer *pl )
{
		...
		debug_file.OpenFile();
		debug_file << "void CHalfLifeMultiplay :: InitHUD\n";
		debug_file.CloseFile();
	...
	if(m_MatchStart.isMatch())
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgHudStatus, NULL, pl->edict());
		WRITE_BYTE (0); 
		MESSAGE_END();
 		bRecovered = m_MatchStart.m_Players.AttemptedToRecover(pl->edict());
	}
	...
	MESSAGE_BEGIN( MSG_ONE, gmsgResetHUD, NULL, pl->pev );
		WRITE_BYTE( 0 );
	MESSAGE_END();

	//if a timer is occuring, let the new client know
	MESSAGE_BEGIN( MSG_ALL, gmsgTimer, NULL);
		WRITE_SHORT (m_dRoundTimer.GetTimeLeft()); // send time left, not time limit
	MESSAGE_END();
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( plr )
		{
			...
			SendScoreInfo(plr, pl);
			
			// send icon status from every player to me
			MESSAGE_BEGIN( MSG_ONE, gmsgIconInfo, NULL, pl->edict());
				WRITE_BYTE( plr->entindex() );
				WRITE_BYTE( 0 );
				WRITE_BYTE( plr->m_isSpectator);
				WRITE_BYTE(	plr->m_bIsRemoved);
				WRITE_BYTE(	plr->m_nCtfFlagInHand );
			MESSAGE_END();

			cvar_t *p_sv_lan = CVAR_GET_POINTER( "sv_lan" );

			// send me his wonid
			MESSAGE_BEGIN( MSG_ONE, gmsgPlayerID, NULL, pl->edict() );  
				WRITE_BYTE( plr->entindex() );
				WRITE_STRING( p_sv_lan->value ? "0" : CVirtualAdmin::GetID(plr) );
			MESSAGE_END();

			// send him my wonid
			MESSAGE_BEGIN( MSG_ONE, gmsgPlayerID, NULL, plr->edict() );  
				WRITE_BYTE( pl->entindex() );
				WRITE_STRING( p_sv_lan->value ? "0" : CVirtualAdmin::GetID(pl) );
			MESSAGE_END();

		}
	}

	pl->m_iVote = VOTED_NONE;
...
//			pPlayer->pev->deadflag = DEAD_DEAD;  // ghost bug.. maybe?
// hle
//			clean up voting stuff
			g_pVote->PlayerDisconnect(pPlayer);
			...
			if(pPlayer->m_isSpectator)
			{
				for (int i = 1; i <= gpGlobals->maxClients; i++ )
				{
					...
					{
						CBasePlayer *pTarget = (CBasePlayer *)pEnt;
						if(pTarget->m_lObservers.lookup(pPlayer->edict())>=0) // two way check
						{
							pTarget->RemoveSpectator(pPlayer->edict());	
						}
					}
				}
			}
			...
			while (pEntity)
			{
				CSpore* pSpore = (CSpore*)pEntity;
				if(pSpore->pev->owner == pPlayer->edict())
				{	
					if (pEntity->IsAlive())
					{
			...
			/////////////////
				...
				//pPlayer->StopObserver(); // this clears other clinets list for hud messagse
			...
			// if we are looking chasing someone we need to remove ourselfs from thier list
			if(pPlayer->pev->iuser2)
			{
				CBasePlayer *pPlayerTarget;
				CBaseEntity *pEnt = UTIL_PlayerByIndex(pPlayer->pev->iuser2);
				if ( pEnt && pEnt->IsPlayer( ) )
				{
					pPlayerTarget = (CBasePlayer *)pEnt;
					if(pPlayerTarget->m_lObservers.lookup(pPlayer->edict())>=0) // two way check
						pPlayerTarget->RemoveSpectator(pPlayer->edict());	
				}
			}
			// end hle
...
float CHalfLifeMultiplay :: FlPlayerFallDamage( CBasePlayer *pPlayer )
{
	...
	//{
	//case 1://progressive
	// force falldamage 1 for beta, death veloctity has been increased from 1000 to 3000
...
void CHalfLifeMultiplay :: PlayerThink( CBasePlayer *pPlayer )
{
	if(m_MatchStart.isMatch())
	{
		if(m_MatchStart.m_bInProgess)
		{
			...
			else if(pPlayer->m_bInMatch && pPlayer->m_isSpectator)
			{
	...
	if(pPlayer->m_bUnSpawn && !pPlayer->m_isSpectator )
	{
		{
...
BOOL CHalfLifeMultiplay::IsAllowedToSpawn( CBaseEntity *pEntity )
{
	...
	char szBannedEntitys[1024];
	*szBannedEntitys = NULL;
	
	strcpy(szBannedEntitys,banentities.string);
	char* pTok = strtok( szBannedEntitys, ";" );

	BOOL bReturn = TRUE;
	while ( pTok != NULL && *pTok )
	{
		if(*pTok == '~')
		{
			pTok++;
			int len = strlen(pTok);
			const char* tmp = STRING(pEntity->pev->classname);
			if(!strncmp(pTok, tmp ,len))
			{
				return TRUE;
			}
		}
		if(!strncmp(pTok, STRING(pEntity->pev->classname),strlen(pTok)))
		{
			bReturn = FALSE;
		}
		pTok = strtok( NULL, ";" );
...
//=========================================================
//=========================================================
edict_t *CHalfLifeMultiplay::GetRandomSpawn( CBasePlayer *pPlayer )
{
	return m_pSpawnPoints.FindRandomSpawnPoint(pPlayer);	
}


edict_t *CHalfLifeMultiplay::GetPlayerSpawnSpot( CBasePlayer *pPlayer ) // hle edit
{
	//edict_t *pentSpawnSpot = CGameRules::GetPlayerSpawnSpot( pPlayer );	
	//edict_t *pentSpawnSpot = EntSelectSpawnPoint( pPlayer );
	edict_t *pentSpawnSpot = g_pGameRules->GetRandomSpawn(pPlayer);
	if(!pentSpawnSpot)
	{
		CBaseEntity* pSpawnPoint = UTIL_FindEntityByClassname( NULL, "info_player_start");
		if ( !FNullEnt(pSpawnPoint->edict())) 
		{
			ALERT(at_error, "PutClientInServer: no info_player_start on level");
			pentSpawnSpot = INDEXENT(0);
			
		}
		else
			pentSpawnSpot = pSpawnPoint->edict();
...
//	ShowGameInfo(NULL, true, 5);
	...
	m_flIntermissionEndTime = gpGlobals->time + 1;//( (int)mp_chattime.value ); // hle 4
...
void CHalfLifeMultiplay :: ChangeLevel( void )
{
	...
	// hle
    g_pVote->ResetVoteVars(); 
	// end hle

	// find the map to change to

	char mapcfile[64];
	sprintf(mapcfile, "modes/%s/", gamemode.string);
	strcat(mapcfile, (char*)CVAR_GET_STRING( "mapcyclefile" ));
	...
	*szNextMap=NULL;
	...
	if ( do_cycle && mapcycle.items )
	{
		...
		for ( item = mapcycle.next_item; item->next != mapcycle.next_item; item = item->next )
		{
			...
			if(FStrEq(item->mapname, STRING(gpGlobals->mapname)))
			{
				keeplooking = TRUE;
			}
	...
	// move this later
 	if(strcmp(nextmap.string, "") && overwrite_nextmap.value==0)
	{
		strncpy(szNextMap, nextmap.string, 31);
		CVAR_SET_STRING(nextmap.name, "");
	}
	...
	if ( strlen( szCommands ) > 0 )
	{
		SERVER_COMMAND( szCommands );
			...
			debug_file.OpenFile();
			debug_file << szNextMap << endl;
			debug_file << "void CHalfLifeMultiplay :: ChangeLevel( void )\n";
			debug_file.CloseFile();
...
BOOL CHalfLifeMultiplay::CanPlayerSpawn( CBasePlayer* pPlayer )
{
	return TRUE;
}


void CHalfLifeMultiplay::CapturedFlag(CBasePlayer *pPlayer)
{

}

BOOL CHalfLifeMultiplay::RestrictObserver( void )
{
	return FALSE;
}
...
{ 
	GoToIntermission();  // hle

//	HLE_EndGame();
}


BOOL CHalfLifeMultiplay::CanStartObserver(CBasePlayer *pPlayer)
{
	if(m_MatchStart.isMatch())
	{
...
int CHalfLifeMultiplay::CheckValidTeam(char* pCurrentTeam, char* pNewTeam)
{
	return 1;
}
BOOL CHalfLifeMultiplay::CanStopObserver(CBasePlayer *pPlayer)
{
	if(m_MatchStart.isMatch())
		return FALSE;
	return TRUE;
...
void CHalfLifeMultiplay::MatchStart(int nType)
{
	m_MatchStart.setMatch(nType);	
}

BOOL CHalfLifeMultiplay::MatchInProgress(void)
{
	return m_MatchStart.isMatch();
}

BOOL CHalfLifeMultiplay::CanStartAutoObserver(CBasePlayer* pPlayer)
{
	if(m_MatchStart.isMatch())
		return FALSE;
	return TRUE;
...
void CHalfLifeMultiplay::RemovePlayer(CBasePlayer* pPlayer)
{
	pPlayer->m_bIsRemoved = true;
	if(g_pGameRules->CanStartObserver(pPlayer)) // handles a few teamplay issues
...
void CHalfLifeMultiplay::ResumePlayer(CBasePlayer* pPlayer)
{
	...
	MESSAGE_BEGIN( MSG_ALL, gmsgIconInfo );
		WRITE_BYTE( pPlayer->entindex() );
		WRITE_BYTE( GetTeamIndex( pPlayer ) + 1 );
		WRITE_BYTE( pPlayer->m_isSpectator );
		WRITE_BYTE(	pPlayer->m_bIsRemoved );
		WRITE_BYTE(	pPlayer->m_nCtfFlagInHand );
	MESSAGE_END();
...
void CHalfLifeMultiplay::ResetPlayerHud(CBasePlayer* pPlayer)
{
	MESSAGE_BEGIN( MSG_ONE, gmsgReset, NULL, pPlayer->edict() );  
	MESSAGE_END();
...
void CHalfLifeMultiplay::SetSpectatorHud(CBasePlayer* pPlayer, CBasePlayer* pTarget)
{
	MESSAGE_BEGIN( MSG_ONE, gmsgColor, NULL, pPlayer->edict() );  
	WRITE_BYTE( pTarget->m_nHue );
	WRITE_BYTE( pTarget->m_nValue );
	WRITE_BYTE( -1 );
	MESSAGE_END();
}
...
{
	int nColor;
	//if(FStrEq(g_engfuncs.pfnInfoKeyValue( infobuffer, "gauss" ), "lms"))
	//	pPlayer->m_nGaussColor=9;		// radnom number to prevent client side dubplication

	nColor = atoi(g_engfuncs.pfnInfoKeyValue( infobuffer, "gauss" ));
	if(nColor < 0)
...
void CHalfLifeMultiplay::AddClock(float time)
{
...
void CHalfLifeMultiplay::GivePlayerBonus(CBasePlayer* pPlayer)
{
	
	int addlife = pPlayer->pev->max_health - pPlayer->pev->health;
	int addhev  = pPlayer->max_hev - pPlayer->pev->armorvalue;
...
void CHalfLifeMultiplay::SendScoreInfo(CBasePlayer* pSender, CBasePlayer* pReciever)
{
	if(pReciever==NULL)
	{
...
void CHalfLifeMultiplay::StartObserver(CBasePlayer *pPlayer)
{
