...
//extern CDelayTime g_A3dDelay;
extern CVirtualAdmin g_vAdmin;

int g_nMaxTeams;

#ifdef HLE_DEBUG2
COutput debug_file("hle/file_dump.txt");
#endif
//COutput debug_file("hle/file_dump.txt");
...
public:
	CWorld()
	{
		...
		debug_file << "======== START HLE ========\n";	
		debug_file << (char*)STRING(gpGlobals->mapname) << endl;
		...
		{
			team_order[i]=i; // BLUE, RED, GREEN, etc...
		}
	}
	void Spawn( void );
	void Precache( void );
...
static void InitBodyQue(void)
{
	...
	for ( int i = 0; i < corpsequeue.value; i++ )
	{
...
//	if ( IS_DEDICATED_SERVER() )
	{
		if(!g_vAdmin.m_bLoaded)
		{
			g_vAdmin.LoadPermissionGroups();
			g_vAdmin.WriteToDisk("hle/SAVE/VADMIN_BACKUP.txt"); // save at this step
		}
		else
		{
			g_vAdmin.WriteToDisk("hle/vadmin.txt"); // save at this step
		}
	}
	...
	g_fTimeOffset = 0; // hle time hack
	
	if (g_pGameRules)
	{
		#ifdef HLE_DEBUG2 // DEBUG FILE DUMP =============
		debug_file.OpenFile();
		debug_file << "Deleting game rules...\n";
		debug_file.CloseFile();
		#endif	// END FILE DUMP =========================
	...
	g_pGameRules = InstallGameRules();
		...
		debug_file.OpenFile();
		debug_file << gamemode.string << endl;
		debug_file.CloseFile();
...
{
	if ( FStrEq(pkvd->szKeyName, "gamemode") )
	{
		g_nMapMode = atoi(pkvd->szValue);
		m_bInitMode = true;
	}
	if ( FStrEq(pkvd->szKeyName, "maxteams") )
	{
		g_nMaxTeams = atoi(pkvd->szValue);
		m_bInitTeams = true;
	}
	if ( FStrEq(pkvd->szKeyName, "blue_team") )
	{
		team_order[BLUE-1] = atoi(pkvd->szValue)-1;
	}
	if ( FStrEq(pkvd->szKeyName, "red_team") )
	{
		team_order[RED-1] = atoi(pkvd->szValue)-1;
	}
	if ( FStrEq(pkvd->szKeyName, "green_team") )
	{
		team_order[GREEN-1] = atoi(pkvd->szValue)-1;
	}
	if ( FStrEq(pkvd->szKeyName, "yellow_team") )
	{
		team_order[YELLOW-1] = atoi(pkvd->szValue)-1;
	}
	if ( FStrEq(pkvd->szKeyName, "purple_team") )
	{
		team_order[PURPLE-1] = atoi(pkvd->szValue)-1;
	}
	if ( FStrEq(pkvd->szKeyName, "orange_team") )
	{
		team_order[ORANGE-1] = atoi(pkvd->szValue)-1;
	}
